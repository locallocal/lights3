...
> English translation of [../../storage/cloudproxy-design.md](../../../architecture/storage/cloudproxy-design.md). The Chinese original is authoritative; section numbering matches.
...
> carries forward the overview in docs/architecture/storage/storage-backend.md §4 and the reservation
> in docs/architecture/storage/tiered-design.md §10 P5 (hooking tiered's cloud side up to a real
...
> docs/architecture/storage/storage-backend.md §4.1).
...
| Implement the full `IStorageBackend` interface | Including bucket CRUD, the object data plane, list, and the complete multipart set (docs/architecture/storage/storage-backend.md §1, with `src/storage/backend.h` as the source of truth) |
| End-to-end streaming | GET/PUT never buffer a whole object in memory; backpressure propagates down to the TCP layer (continuing the principle of docs/architecture/storage/storage-backend.md §4) |
...
| Support tiered P5 | Serves as the cloud-side backend of docs/architecture/storage/tiered-design.md's TieredBackend (the primary consumer); acceptance checklist in §9 |
...
- No caching of remote data—caching is the responsibility of TieredBackend
  (docs/architecture/storage/tiered-design.md §6); separation of concerns;
...
docs/architecture/storage/storage-backend.md §4.1 originally preferred route A (wrapping
...
4. **A controllable threading model**: a self-built implementation can be fused
   ...
   (docs/architecture/storage/storage-backend.md §4 already pointed out the thread-hogging bottleneck
   of a synchronous SDK).

Costs: the credential chain had to be written by hand (done, see §1/§7,
`aws_credentials.h`); S3
...
The generic per-backend dedicated pool reserved in docs/architecture/concurrency.md §3.1
...
③ Once ① has the meta: co_return ObjectStream{meta,
       make_unique<PumpBodyReader>(queue, len /* cancels on drop */), range}
```
...
- nullopt (true chunked with no length): AWS S3 does not accept bare
  ...
  back to `NotImplemented` (`cloudproxy_backend.cc` `spool_and_upload`).
...
- **Utility gap**: `LastModified` in the response is ISO8601, and
  `core/util/time.h` only had the `iso8601()` formatter—P1 added
  `parse_iso8601()`;
...
| --- | --- |
| 4xx + parsable XML `<Error><Code>` | Reverse-lookup `S3ErrorCode` by wire code and pass through verbatim (NoSuchKey / NoSuchBucket / NoSuchUpload / InvalidRange / InvalidPart / BucketNotEmpty / BucketAlreadyOwnedByYou / EntityTooLarge…); the reverse lookup is `code_from_wire()` in `s3/errors.h` |
...
| 500 / 502 / 504, 5xx with unparsable body | `InternalError` (local 500). **No 502 introduced**: the S3 error vocabulary has no BadGateway; standard S3 clients treat 500/503 as retryable; stay protocol-faithful (the original "502/503" wording in docs/architecture/storage/storage-backend.md §4 is revised along with this document) |
...
- Single-part PUT / upload_part: remote ETag = content MD5. §3.2 already
  ...
  docs/architecture/storage/tiered-design.md §5.2's dependency of "verify the cloud-returned etag
...
abort_multipart / list_parts / list_uploads / head_part / put_tagging /
delete_tagging; data plane get / put / upload_part. op/code-dimension instances are registered on demand through a
...
## 9. Acceptance for Integration with TieredBackend (docs/architecture/storage/tiered-design.md P5)

As tiered's cloud-side backend, the acceptance checklist:

1. After `put_object` uploads with `user_meta` (the `x-amz-meta-lights3-*`
   redundancy headers, docs/architecture/storage/tiered-design.md §4.2), `head_object` /
...
2. The etags returned by put / upload_part / complete are non-empty;
   ...
   docs/architecture/storage/tiered-design.md §5.2 step ③);
...
4. head returns size / etag / last_modified in full (the conditional-request
   dependency of docs/architecture/storage/tiered-design.md §6.1);
5. `list_objects` is usable for the docs/architecture/storage/tiered-design.md §9 reconciliation
   traversal;
6. When the remote is unreachable, throw `InternalError` / `SlowDown` rather
   than hanging (docs/architecture/storage/tiered-design.md §9's fault matrix relies on
...
`verify()` (mirror images of each other—a free regression). The test uses port=0
auto-selection.
...
| P4 | Retry/backoff, timeout refinement, metrics, logging; decision on the length-less body path (spool to disk then fixed-length upload, no TRAILER framing, §3.2); `control_in_pump` default set by load test | Fault-injection targeted tests pass | ✅ All landed (2026-07-31): metrics in §8.2; `control_in_pump` load test set default false (§2.3); `force_path_style: false` vhost implemented along the way (§7) |
| P5 | e2e dual-instance script; tiered integration (§9 checklist); docs/architecture/storage/tiered-design.md P5 status update | e2e passes; tiered + cloudproxy smoke test passes | ✅ (`e2e_cloudproxy` + `e2e_tiered_cloudproxy`) |
