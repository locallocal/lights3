...
> English translation of [../concurrency.md](../../architecture/concurrency.md). The Chinese original is authoritative; section numbering matches.
...
| `core/executor.h` | `IExecutor` posting abstraction, `InlineExecutor`, `PumpExecutor`, `resume_on` |
...
Three implementations + one switching primitive:
...
2. **ThreadPoolExecutor**: `post` = wrap the resume into a task and enqueue it
   unbounded into the thread pool. `Application` (`src/app/app.cc`) uses it as
   the posting target for semaphore wakeups (§6);
3. **PumpExecutor**: a synchronous driver's connection/request thread pumps a
   queue while it waits in `sync_wait_pumping`; `post` delivers onto that thread
   ([coroutine-internals.md §2.2](coroutine-internals.md));
4. `co_await resume_on(ex)`: switches the rest of the current coroutine onto
...
- `join()`: stops accepting new tasks, **drains the queue** (including the
  backlog), then waits for the threads to exit; `schedule` after join throws,
  `post` does not (its consumers are mostly noexcept paths) — it logs ERROR and
...
Thread-per-request: the request thread runs
`resp = sync_wait_pumping(exec, handler_(std::move(req)))` and, while waiting,
doubles as a `PumpExecutor` pumping its queue — blocking body reads are moved
back onto the request thread via `co_await resume_on(exec)`, so a slow client
only drags down its own thread and never occupies the shared pool; other child
tasks that complete on a pool thread advance the continuation in place to the
next suspension point or to completion, and `exec.finish()` ends the pump loop
when the top-level task completes.

Streaming responses work the same way: builtin's whole response loop is one
coroutine driven by `sync_wait_pumping`, and httplib's content provider collects
each chunk through `StreamPrefetch::next_sync()` (both double-buffer prefetch,
[http-adapter.md §2.4](http-adapter.md) ①⑤). httplib's request body is a push model,
...
A process-level, two-threaded timer: a scheduling thread only decides expiry,
a dedicated callback thread runs callbacks serially
([coroutine-internals.md §7](coroutine-internals.md)). `add(delay, fn)` returns
an id, `cancel(id)` retracts a not-yet-fired timer (a callback already due but
still running is waited for). Callbacks run serially on the callback thread and
**must be lightweight** —
...
| Site | Purpose |
| --- | --- |
| dispatch entry (`http/admission.h`, wired by `src/app/app.cc`) | `runtime.max_inflight_requests` global throttling; over-limit requests queue rather than being rejected; waiters woken via the pool executor. For streaming responses the Permit is tied to `stream_body` (outermost wrapper) and released only when the driver finishes or discards the body — throttling covers the whole response transfer, not just the handler coroutine frame; shutdown draining judges in-flight via `available()`, so it counts mid-stream requests too |
...
exactly one place), now shared by duostore, tiered, localfs, CredentialStore
(credential-management.md §10.3), the website / usage / lifecycle /
sys_config stores, the scrub throttle and the tables maintenance runner. The problem it solves: timer-driven
