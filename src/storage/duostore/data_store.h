...
// (batching — per-record delivery costs one fdatasync + one
...
// an in-flight large object as unreferenced files
...
struct IDataStore {
    ...
    // Batch write (compaction migration only): K payloads
    ...
    // Age-based rotation: seals active packs whose first
    ...
    // metric accumulation: enumeration needs a stat anyway, so
    ...
    // Reverse reconciliation enumeration of packs/: the pack
    ...
    // sealing). The fs implementation probes the active pack's
    ...
    // Actual pack file size: accounts left as seal(0) by a
