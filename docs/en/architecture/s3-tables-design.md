...
> the sections here, and the loose ends and long-term items are in [todo.md](../development/todo.md)
...
> source comments cite it as `docs/architecture/s3-tables-design.md §N`.
>
...
[deployment.md](../usage/deployment.md) §5 (multi-gateway support matrix), [cli.md](../usage/cli.md) (AdminJobs and `lights3-ctl`).
...
REST extension `PUT /iceberg/v1/buckets/{bucket}` and the CLI
`lights3-ctl tables enable <bucket>` (which calls the former) share one
implementation; the admin plane `/-/admin/tables/…` carries only the per-table
jobs of §9. Preconditions:
...
struct TableEntry {
    ...
    std::string version_token;       // "t-" + 32 hex chars (16 getentropy bytes); renewed per commit
...
.sys/tables-catalog/<bucket>/ns/<l1>/<l2>/view/<view>.json    ViewEntry (⑥)
.sys/tables-catalog/<bucket>/maint/<l1>/<l2>/<table>.json     MaintenanceConfig (④, §16)
```
...
In-process, a per-(bucket, table_id) `AsyncSemaphore(1)` is added as a fast path:
...
[deployment.md §5.1](../usage/deployment.md): duostore (redis / tikv meta + rados data)
...
deployment matrix is in [deployment.md §5.1](../usage/deployment.md).
...
| load / exists table, metadata-location GET | (w, "<ns-path>/<t>") | Read |
...
| POST `/{w}/namespaces/{ns}/tables/{t}/metrics` | ReportMetrics: parsed, written to the audit log as `tables.metrics`, 204 (§16 ⑥) | ⑥ |
...
(not a 501 XML): `stage-create`, register with `overwrite: true`, format-version 3,
`purgeRequested=true` without a job framework. Unimplemented paths
(`/transactions/commit`, `/plan`, `/tasks`, `/sign`, `/oauth/tokens`, …) have no
route and answer 404 `NoSuchResourceException`; a wrong method on a known path
answers 405 `MethodNotAllowedException`.
...
- **LoadTable**: `?snapshots=all|refs` (`refs` prunes to snapshots referenced by
  ...
  `{"s3.path-style-access":"true","lights3.credential-vending":"disabled|supported",
  "lights3.credential-scope":"table-prefix","lights3.table-location":"<location>",
  "lights3.version-token":…,"lights3.snapshot-validation":…,"lights3.catalog-etag":…}`
  (`s3.endpoint` / `s3.region` are not given: the gateway does not know its external address and
...
`check_range`; everything is **restart-only** (listed in `docs/usage/config-reload.md §4`
...
- Sources: `src/tables/{catalog_store.h, object_catalog_store.cc, duo_meta_catalog_store.cc,
  catalog.cc, identifier.cc, table_bucket_store.cc, rest_api.cc, rest_error.h, bucket_guard.cc,
  diagnostics.cc, fsck.cc, maintenance.cc, maintenance_runner.cc,
  iceberg/{metadata.cc, requirements.cc, updates.cc, transition.cc, snapshots.cc, avro_reader.cc,
  manifest.cc, view_metadata.cc}}`,
...
- CLI: `lights3-ctl tables enable|disable|status <bucket>`, `tables list <bucket>`,
  `tables config|plan|run|purge`, `tables diagnose|recover [--prune]`; `lights3 fsck` gains a "catalog state vs. table
...
`lights3 tables export|import` migrates between the two backings ([cli.md §2.6](../usage/cli.md)).
...
| multi-table transactions `/transactions/commit` | needs cross-object atomicity, which object CAS cannot give; the duostore-meta backing (§12) landed with ⑥, so the atomicity precondition is met; kept as a long-term item in [todo.md §5](../development/todo.md) |
...
this design are summarized below; open items are in [todo.md](../development/todo.md) §2 / §4 / §5.
