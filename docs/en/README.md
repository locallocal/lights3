...
| [s3-tables-design.md](s3-tables-design.md) | S3 Tables / Apache Iceberg REST Catalog design draft (not implemented): findings from the RustFS implementation, catalog state on `.sys` with a `PutCondition` CAS commit protocol, REST endpoints and error model, table-bucket guard and credential vending, maintenance jobs, implementation steps ①–⑥ |
| `s3-tables/` | S3 Tables implementation documents (not implemented, Chinese only): one implementation-level document per step of design §14 — file list, data structures and signatures, step-by-step flows, integration points in existing code, configuration, unit-test list, pitfalls |
