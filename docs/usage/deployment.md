...
docker run --rm -e LIGHTS3_SECRET_1=x lights3 --version   # 以 - 开头的首参走服务入口，entrypoint 缺 LIGHTS3_SECRET_1 直接退出 2
...
外部依赖，缺则显式 SKIP（[testing.md §1](../development/testing.md)）。compose 的 `e2e` profile
...
"表目录"列只看**默认后端**（目录状态在它的 `.sys` 里，[s3-tables-design.md §5.5](../architecture/s3-tables-design.md)）；
...
| `read_lease` | 所有网关开启（默认 5s，勿关） | 各网关把"最老在途读 / 写开始时间"发布到共享 meta，GC 网关据此推迟回收（[storage/duostore-core.md §8.5](../architecture/storage/duostore-core.md)）；设 `0s` 则须 `gc_grace` ≥ 最长预期 GET **与上传**时长，代码不校验 |
...
| 实例级后台任务（`usage.reconcile`、tiered 扫描等） | 只在一台开 | 同 `gc_enabled` 的指定实例语义（[multi-tenancy.md](../architecture/multi-tenancy.md)） |
| 凭证与 `.sys` | 同一份配置（凭证、区域、密钥） | STS 会话经 `.sys/sts` 写穿共享；静态凭证各网关自持 |

**表目录多网关**（[s3-tables-design.md §5.5](../architecture/s3-tables-design.md)）：`tables.enabled` 与
...
sudo ./scripts/rollback.sh           # 换回 *.prev 并重启；再跑一次又换回来（--no-restart 只换不重启）
...
- **install.sh** 在动任何文件之前先用**新**二进制 `--check-config` 校验现有
  ...
  该 helper 由 install.sh 装到 `/usr/local/share/lights3/lights3-setup.sh`。
- **rollback.sh**：先用 `lights3.prev` 跑 `--check-config`（旧二进制不认识新
  配置键时会被拒，此时先手动回退配置），再三步 rename 交换 `lights3` /
  `lights3-ctl` 与其 `.prev`，服务在运行则 restart（`--no-restart` 跳过重启）。
  配置与数据不动。
- **uninstall.sh**：`remove` → 删文件（含 `/usr/local/share/lights3`）→ `daemon-reload`；
  `--purge` 追加 helper 的 `purge`。包安装的实例用包管理器卸载（§3.1）。
