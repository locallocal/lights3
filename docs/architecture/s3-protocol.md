...
| Tables | Iceberg REST Catalog（`/iceberg/v1/...`，JSON） | ①–⑥ 已落地（2026-09-12；Iceberg views `…/views…` + `views/rename`；`tables.compat_prefix` 别名；`reportMetrics` 记审计 `tables.metrics`；`tables.catalog_backing: duostore` 时提交为 meta 事务；PyIceberg 0.12 / DuckDB 1.5 冒烟通过；通用 SigV4 客户端不带 `x-amz-content-sha256` 也可，目录面自己算 payload 哈希）：维护 `GET/PUT …/tables/{t}/maintenance/config`、`POST …/maintenance/plan|run`（202 + 作业）、`GET …/maintenance/jobs/{id}`、`DELETE …/tables/{t}?purgeRequested=true`（204 + `x-lights3-job-id`）、管理面 `POST/GET /-/admin/tables/<bucket>/<ns>/<t>/plan|run|purge`（root）；CommitTable / RegisterTable 深校验 manifest-list → manifest → 数据文件（Avro null / deflate，其他 codec 跳过并在 `config["lights3.snapshot-validation"]` 报 `skipped-codec`）；`GET …/tables/{t}/catalog/diagnostics`、`POST …/catalog/recovery`；LoadTable 带 `ETag`，`If-None-Match` 命中回 304；目录路径的 SigV4 scope 接受 `s3` 与 `s3tables`（`tables.accept_s3tables_signing`）；per-credential policy 按 (bucket, `<ns-path>/<table>`, action) 判定并过滤列表；租户凭证只见本租户表桶；`X-Iceberg-Access-Delegation: vended-credentials` 得到按表前缀收窄的 STS 会话（`tables.credential_vending`，`GET …/credentials` 同）；表桶排除在 lifecycle 之外。`GET /v1/config`、namespaces 全部端点、tables 的 list / create / load / commit / exists / drop / rename / register、`metadata-location` GET/PUT（AWS `GetTableMetadataLocation` 形态）、表桶启用 `PUT /iceberg/v1/buckets/<bucket>`（root）；表桶内保留前缀 `.lights3-table/` 在 S3 面只读、非空表桶拒绝 DeleteBucket、桶名 `iceberg` 保留；错误为 Iceberg JSON 信封。设计与各步实现记录见 [s3-tables-design.md](s3-tables-design.md)（§16） |
...
- **解析**：需要解析请求 XML 的只有 CompleteMultipartUpload、
  DeleteObjects、CreateBucket(LocationConstraint) 以及 `?cors` / `?lifecycle` /
  `?website` / `?quota` / `?tagging` 的 PUT 配置体。结构都很浅，用
...
`NoSuchQuotaConfiguration`(404)、`NoSuchTenant`(404)、`ScrubInProgress`(409，admin fsck 并发触发)、`JobInProgress`(409，admin duostore/tier 后台轮次并发触发)、`TenantAlreadyExists`(409)、
...
- **条件请求**：GET/HEAD/PUT 支持 If-Match / If-None-Match，
  ...
  **原子性范围**：条件判定与提交同处后端自身的原子提交点（localfs 提交段
  ...
  （[architecture.md](overview.md) 的非目标：数据面单实例假设）。
...
- **sink 与异步**：`log.file` 为空写 stderr，否则写按大小轮转的文件
  ...
  不受这些开关影响。
- **Metrics**（Prometheus 文本格式，`GET /-/metrics`，默认与数据面同一监听，
  配置 `http.admin_port` 后只在独立 admin 监听上提供、数据面端口回 404
  （[http-adapter.md §2.1](http-adapter.md)）；
