...
> English translation of [../../storage/storage-backend.md](../../../architecture/storage/storage-backend.md). The Chinese original is authoritative; section numbering matches.
...
`list_multipart_uploads()`; `put_object` takes a trailing `PutCondition cond = {}`
(conditional writes); there are optional hooks with default implementations
(`copy_object_fast` / `resolve_object_part` / `set_object_tagging` /
`inspect_object`); `close()` has a default empty implementation rather
than being pure virtual. The per-method contract is in
[common.md](../../../architecture/storage/common.md).)
...
- Lock-free reads; the rule table is swapped as a whole on config hot reload
  (backend hot add / remove travel in the same snapshot; `default_backend`
  cannot change at runtime), see [config-reload.md](../../usage/config-reload.md)
  and [common.md](../../../architecture/storage/common.md). A rule `match`
  starting with `!` negates the glob.
...
- **Metadata cache** (roadmap §3.8): HEAD/GET consult a (bucket, key)-sharded
  ...
  [storage/localfs.md](../../../architecture/storage/localfs.md) §5.1.
...
- **LIST**: recursive directory walk + prefix pruning (a prefix containing `/`
  ...
  no longer cost more than early ones. See [storage/localfs.md](../../../architecture/storage/localfs.md)
  §6.
...
(rationale in docs/architecture/storage/cloudproxy-design.md §2.1): outbound signing
...
Key points (expanded in the corresponding sections of docs/architecture/storage/cloudproxy-design.md):
...
- **Streaming**: in the GET direction, a pump thread + bounded queue turns
  ...
  symmetric inverse. Avoids whole-object buffering (docs/architecture/storage/cloudproxy-design.md §3).
...
- **Timeouts and retries**: connect/request timeouts, exponential backoff 3 times;
  ...
  4xx semantics pass through (NoSuchKey etc.) (docs/architecture/storage/cloudproxy-design.md §5).
...
- Thread occupancy: a synchronous HTTP client holds a thread for the entire
  ...
  rather than the shared pool (docs/architecture/storage/cloudproxy-design.md §2.3).
...
[storage/duostore-core.md](../../../architecture/storage/duostore-core.md) §7.1.
