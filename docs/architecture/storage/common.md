...
接口定义：`storage/backend.h: IStorageBackend`。除五处带默认实现
（`copy_object_fast` / `resolve_object_part` / `inspect_object` 返回 `nullopt`、
`set_object_tagging` 抛 `NotImplemented`(501)、`close` 空操作）外全部为纯虚。
...
**`resolve_object_part(bucket, key, part_no)`** — GET `?partNumber` 支持：返回
`ObjectPartExtent{offset, size, parts_count}`。默认 `nullopt`，L2 回退到
`ObjectMeta::part_sizes` 记录的布局；布局在远端的转发型后端（cloudproxy）
覆写为远端查询。

**`set_object_tagging(bucket, key, tagging)`** — PUT/DELETE `?tagging` 原地
改写标签集（规范 URL 编码形式，空串 = 清空），不重写数据。默认抛
`NotImplemented`(501)——元数据随数据一起原子提交的引擎（duostore）没有原地改
元数据的原语；PUT/CreateMultipartUpload 时带的 `x-amz-tagging` 各后端都持久化。
memory/localfs/tiered/cloudproxy 有覆写。

**`inspect_object(bucket, key)`** — 运维自省（`lights3-ctl object inspect` /
`GET /-/admin/objects/<bucket>/<key>`）：返回 `ObjectLayout{engine, attrs, extents}`
（extent kind ∈ file/chunk/pack/rados，带 id/offset/length/crc32c）。默认
`nullopt`（memory/cloudproxy 无内部布局可展示）；bucket/key 不存在同
`head_object` 抛错。
...
七个一等字段（Cache-Control / Content-Disposition / Content-Encoding /
Content-Language / Expires / x-amz-website-redirect-location / x-amz-tagging）
由常量表 `storage/backend.h: kStdMetaFields` 统一驱动：每项记录请求/响应头名、
持久化键名、`ObjectMeta` 成员指针与 `echo` 标志（`tagging` 为 `echo=false`：
随表提取/持久化但不作为响应头回显，GET 只答 `x-amz-tagging-count`，标签走
`?tagging`）。提取、回显、各后端序列化都遍历这张表——新增字段只改这一处，避免
...
- `ListOptions{prefix, delimiter, max_keys, start_after}`：delimiter 为任意
  字符串（空 = 不分组），分组是 prefix 之后的通用子串查找（§6.2）；
...
- `ListUploadsOptions{prefix, delimiter, max_uploads, key_marker, upload_id_marker}`：
  prefix/delimiter 语义同 `ListOptions`；`(key_marker, upload_id_marker)` 构成复合
  游标，只返回严格大于该二元组的条目；`upload_id_marker` 为空表示
  "key > key_marker"。
...
`PutResult{etag, checksum_algorithm, checksum_value, checksum_type}`（后三项由
complete 在算出 COMPOSITE 校验和时填入，处理器回显进响应 XML）、
`BucketInfo{name, created}`、
`PartInfo{part_no, etag, checksum_algorithm, checksum_value}`
（CompleteMultipartUpload 请求项，etag 可能带引号，比较前用
`strip_etag_quotes` 剥掉；校验和为客户端声明值，L2 对照已存片值校验、不作来源）、
`PartChecksum{algorithm, value, pending}`（UploadPart 随 body 携带的已验证片校验和，
`resolved()` 取 value 或 trailer 捕获槽）、`PartMeta`（ListParts 结果项，多
size、last_modified 与上传时记录的 `checksum_algorithm/checksum_value`）、
`UploadInfo{key, upload_id, initiated}`。
...
|---|---|
| `localfs` | `fs_backend_paths` 解析 `root`（必填，缺失抛 runtime_error）与 `staging`（默认 `<root>/.lights3-staging`）；`fs_backend_opts` 解析 `mpu_ttl` / `mpu_scan_interval` / `require_xattr` / `sidecar` / `list_meta_concurrency`（∈ [1,256]）/ `list_cache_entries` / `list_cache_min_dir_entries` / `sidecar_scan_interval` / `meta_cache_entries` / `meta_cache_ttl` / `meta_cache_validate`（语义与默认值见 [localfs.md](localfs.md) §2/§5.1/§6） |
...
（[storage-backend.md](storage-backend.md) §6）。`StorageRegistry::registered_types()`
返回已注册 type 名（排序，额外含 `tiered`），供 `lights3 --check-config` 不开
任何后端就校验 `backends[].type`。

### 4.2 build 的两阶段与回滚

`storage/registry.cc: StorageRegistry::build(configs, pool, metrics, existing)`
步骤（第四参 `existing` 是后端热添加时已在运行的实例表，backlog ⑦：只供
查找，不返回、不重建）：

1. **重名检查**：backend name 在本批内重复、或与 `existing` 中的名字撞车，直接抛。
...
4. **第二阶段（复合后端）**：`deferred` 上做不动点迭代——某条 tiered 的
   ...
   查找经 `lookup` 先看本批 `out`、再落到 `existing`），
...
`storage/bucket_router.h: BucketRouter` 只持一个 `shared_ptr<Shared>`：
`Shared{default_backend, default_name, atomic<shared_ptr<const Table>> table}`，
`Table{rules, backends}` 是规则表 `Rule{glob, negate, backend, backend_name}` 与
全量后端表的一份不可变快照。`resolve(bucket)` 取快照后按声明顺序线性扫描，
`glob_match(rule.glob, bucket) != rule.negate` 即命中（异或实现取反规则），
全不命中落到 `default_backend`。读路径无锁；`glob_match` 用 `::fnmatch`，
...
两者都拒绝改 `default_backend`（它承载 `.sys` 与由其加载的各 store）。
`app.cc` 的重载路径先 `StorageRegistry::build(..., existing)` 建新实例、
`meter_backends` 包装，再 `router().update(target, candidate)`。
辅助读：`backends()` 返回后端集快照（重载期间迭代仍稳定）、
`backend_name(bucket)` / `default_backend_name()`（指标 backend 标签）、
`rule_count()`。
...
- `composite_checksum(algorithm, part_values_b64)`：S3 COMPOSITE 校验和——各片
  base64 值解回原始摘要按序拼接，再按算法整体求摘要（SHA1/SHA256 走
  `HashStream`，CRC32/CRC32C 取大端 4 字节），`base64 + "-" + N`。算法不支持
  组合（CRC64NVME 仅 full-object）、任一片 base64 解码失败或摘要长度不一致时
  返回 `nullopt`——调用方宁可不记也不记错。
- `apply_composite_checksum(parts, meta, result)`：各后端 complete 的共用钩子。
  `parts[i]` 是第 i 片存储时记录的 `PartDigest{algorithm, value}`；仅当每片都
  带同一算法的已验证值时才填 `meta.checksum_*`（type=`COMPOSITE`，同时清
  `checksum_pending`）与 `result.checksum_*`，否则两者原样不动。
- `join_part_sizes` / `parse_part_sizes`（声明在 `backend.h`，实现在此）：
  `part_sizes` 的文本线格式，逗号分隔十进制；解析遇畸形整体返回空（= 未知）。
- 常量 `kMaxParts = 10000`、`kMinPartSize = 5MiB`：最小片大小只在 complete
  时判且属 S3 协议规则而非存储规则，检查放在 L2（`handlers/multipart.cc`，
  阈值读 `S3Service::min_part_size()`，来自配置 `http.min_part_size`，默认即
  `kMinPartSize`），存储层不设限——直接使用后端 API 的调用方（含各后端一致性
  测试套件）不受 5MiB 约束。
...
- 公共层自身登记的指标：`lights3_backend_pool_*` 四项（§4.3）、
  ...
  （§4.1）；`MeteredBackend`（§1）的 `lights3_backend_op_seconds{backend,op}`
  直方图与 `lights3_backend_errors_total{backend,op}`（非 S3 4xx 的失败才计错：
  HEAD 的 NoSuchKey 是客户端的事，5xx 与传输异常才是后端的）——计时取调用
  墙钟：put/upload_part 含灌入 body 的时间，get_object 只含打开、字节随后经
  driver 流出；`MetaCache`（§1）的 `lights3_meta_cache_lookups_total{result=hit|miss|stale}`、
  `lights3_meta_cache_invalidations_total`、`lights3_meta_cache_entries`。
  后端内部指标接入示例见 [duostore-design.md](duostore-design.md)。
