...
// (docs/architecture/storage/duostore-design.md §2.1: "S3 semantics duplicated across implementations
...
// Per-item extent cap for gcq entries: when deleting TB-scale
...
// ---- Unified schema-marker check (the four engines share the
...
inline ObjectRec assemble_completed_object(ObjectMeta meta, std::span<const PartInfo> parts,
    ...
    // Composite checksum from stored, verified per-part values; the
