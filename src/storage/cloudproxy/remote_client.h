...
// 错误映射与重试（docs/09 §2.2/§5/§8.1）。包含 httplib，只允许被
...
// SigV4 的 UNSIGNED-PAYLOAD 字面值（docs/09 §3.2）
...
struct Endpoint {
    ...
    std::string signed_host;  // 与 httplib 实际发出的 Host 头逐字节一致（docs/09 §2.2）
...
// 互斥保护的空闲栈式连接池；httplib::Client 非线程安全，独占租借（docs/09 §8.1）
class ClientPool {
public:
...
// 错误映射的 404 上下文（docs/09 §5.1：404 且体不可解析时按操作补语义）
...
struct RemoteContext {
    ...
    // 构造最小 HttpRequest 只为签名，再搬运为 httplib::Headers（docs/09 §2.2）。
    ...
    // 远端错误 → 本地 S3Error（docs/09 §5.1 映射矩阵单点实现）
    ...
    // 连接建立阶段的错误（PUT 类可安全重试的子集，docs/09 §5.2）
...
// 跳过整个 common prefix 组的 start-after 值（docs/09 §4.2）：prefix 用 0xff
