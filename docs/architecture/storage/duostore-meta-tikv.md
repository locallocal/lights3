...
| `T` | `kv_key` | `T<key>`（key 不透明，调用方自定） | `'\x01' ‖ <value>`：KV 门面（S3 Tables 目录后备，§4.1）；首字节 `kKvValueMarker` 让空值可存（client-c 以空串表 absent），ETag 按去标记后的逻辑值算 |
...
| `kv_put_batch` / `kv_delete` | 逐 key `snap_get` `T<key>` 取现 ETag | `check_kv_condition` 在快照上校验；现存 → Put，缺失 → **Insert**；delete 缺失 → muts 空 return false（§4.1） |
...
（docs/architecture/s3-tables-design.md §12；契约见 rocksdb 文档 §6.1）落在 `T` 标签
（§1），value 前置 `kKvValueMarker`（`kv_enc`/`kv_dec`），ETag 用共享的
`meta_store.h:kv_etag`。写侧走 `txn_retry`：`kv_put_batch` 在 start_ts 快照上逐 key
读现值算 ETag → `check_kv_condition` → 现存则 Put、缺失则 Insert（撞已有 key 即
`TikvAlreadyExist`/冲突重试），key 本身在写集内，故无守卫分片也能由 prewrite 写写
冲突兑现 CAS；整批一个事务（目录 commit 记录与指针同批落地）；`kv_put` 是单项批；
`kv_delete` 快照读缺失即 muts 空返回 false，否则单 Del。读侧 `kv_get` = 一次 TSO +
`snap_get`；`kv_scan` = 一个 TSO 上 `[T<prefix|after>, T+1)` 分页 `scan`（页尾 key +
`'\0'` 续页，limit=0 取 1000），跳过 `<= after` 的 key、越出 prefix 即断。
...
- `snapshot()`（在线 dump，roadmap §3.7）：`tikv_meta_store.cc:TikvMetaStore::SnapshotView`
  ...
  状态，代价是视图存活期间集群 safepoint 不得越过 `ver_`（§9 的 `gc_retention`
  须覆盖 dump 时长）。`restore_marker()`（§10）用的就是同一个 TSO 原语。
...
safepoint = now − retention（TSO 域直接减 `ms << 18`；服务 id =
`"lights3-duostore:" + prefix`，同前缀网关互相覆写无害；TTL = `max(3×interval, 60s)`，
停摆网关两轮自动摘除）；②以无限 TTL 顶替 `gc_worker` 服务项（PD 对缺失的 gc_worker 永久占位，
