...
> English translation of [../../storage/duostore-meta-sqlite-design.md](../../../architecture/storage/duostore-meta-sqlite-design.md). The Chinese original is authoritative; section numbering matches.
...
- No sharding/partitioning; ~~the in-process online backup API (`sqlite3_backup`)
  remains not-done~~ — implemented (backlog ⑧: `backup_physical` full backup +
  `sqlite_wal_archive` WAL-segment increments, §6,
  [duostore-meta-sqlite.md](../../../architecture/storage/duostore-meta-sqlite.md) §10).
...
-- KV facade for the S3 Tables catalog (kv_*), created IF NOT EXISTS on every open
CREATE TABLE tc(key BLOB PRIMARY KEY, val BLOB NOT NULL) WITHOUT ROWID, STRICT;
```
...
| `tc` (S3 Tables KV facade) | `tc(key BLOB PRIMARY KEY, val BLOB)` table | created `IF NOT EXISTS` on every open; `kv_*` are single-statement point lookups / prefix scans, see [duostore-meta-sqlite.md](../../../architecture/storage/duostore-meta-sqlite.md) §4.1 |
...
separate `alloc_mu_` small lock (lock order always `alloc_mu_` → `mu_`; `mu_` is
held during the reservation to keep in-process business writers out),
...
an immediate 500**; only past the limit is InternalError thrown.
...
- **Cold backup path** (this section is the reference target of §6.3): after a
  ...
  backup; a direct `cp` while running guarantees nothing. **Online backup (judged
  not-done in S4, implemented in backlog ⑧)**: `backup_physical` takes a full
  consistent snapshot in-process via `sqlite3_backup_init/step/finish`
  (`supports_physical_backup()` = true); configuring `sqlite_wal_archive` starts a
  backup chain: auto-checkpoints are switched off (`wal_autocheckpoint=0`), the
  WAL is archived segment by segment as incremental backups with an explicit
  TRUNCATE checkpoint at close, and `restore_physical` replays the chain; without
  a chain the default checkpoint policy above still applies. External tools
  (`VACUUM INTO` / `.backup`) keep working (WAL mode allows concurrent external
  read-only connections). Details in
  [duostore-meta-sqlite.md](../../../architecture/storage/duostore-meta-sqlite.md) §10.
...
submodule is pinned to a recent release tag (currently `version-3.54.0`). `build.sh`'s
...
`SqliteMetaOptions{path, sync, cache_bytes, pool_size, busy_timeout_ms,
wal_archive, metrics}` aligns with the `RocksMetaOptions` shape; the read-pool size reuses the default ≈ thread pool
...
| S4 | Polish: crash simulation (post-kill WAL replay reconciliation) and consistent-view injection specials, `sqlite3_backup` online-backup evaluation (not done then; implemented in backlog ⑧, §6), `PRAGMA optimize`/checkpoint tuning (`journal_size_limit`, §6), metrics (BUSY/corruption counts), doc status header update | Full ctest matrix green | Done (2026-07-30) |
