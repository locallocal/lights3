...
| Tables | Iceberg REST Catalog（`/iceberg/v1/...`，JSON） | 步骤 ① 已落地（2026-09-12）：`GET /v1/config`、namespaces 全部端点、tables 的 list / create / load / commit / exists / drop / rename / register、`metadata-location` GET/PUT（AWS `GetTableMetadataLocation` 形态）、表桶启用 `PUT /iceberg/v1/buckets/<bucket>`（root）；表桶内保留前缀 `.lights3-table/` 在 S3 面只读、非空表桶拒绝 DeleteBucket、桶名 `iceberg` 保留；错误为 Iceberg JSON 信封。设计与后续步骤见 [s3-tables-design.md](s3-tables-design.md) / [s3-tables/](s3-tables/README.md) |
