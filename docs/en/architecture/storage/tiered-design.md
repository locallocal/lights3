> English translation of [../../storage/tiered-design.md](../../../architecture/storage/tiered-design.md). The Chinese original is authoritative; section numbering matches.
...
docs/architecture/storage/storage-backend.md §2 deliberately stops routing at bucket granularity and
...
- **The local side goes through `tier::ITierLocal`**
  ...
  [storage/tiered.md §11](../../../architecture/storage/tiered.md)). `local` may name a
...
(TierIndex / TierScanner are design-level names: in the code they are the
`AccessTable` in `tier_access_table.h` (used only by the duostore adapter; the
localfs adapter keeps a private map + `atime.tsv` snapshot) plus the `Touch`
...
2. **Space watermark**: when `statvfs` usage > `space_high_watermark` (default
   ...
   stops once the collected candidate bytes reach max(4× the deficit, deficit + 64MiB); the sort key
...
- **In-flight readers are unaffected**: step c is a rename-over; a reader in the
  ...
  docs/architecture/object-read-write-flow.md §3.2);
...
③ Yes → wrap in a TeeCacheReader returned to L2:
...
docs/architecture/object-read-write-flow.md §3.1 as a normal 206. Configurable
...
backends:
  ...
  - name: aws
    type: cloudproxy                  # docs/architecture/storage/cloudproxy-design.md
    endpoint: https://s3.us-east-1.amazonaws.com
    bucket_prefix: lights3-tier-
    # cloud credentials...
  - name: tiered
    type: tiered
    local: localdata                  # localfs/xlocalfs or duostore (§2)
...
| Failure | Behavior |
| --- | --- |
| Cloud unreachable (GET remote) | Pass through the mapped cloud error (aligned with cloudproxy: remote 5xx → InternalError (500) / SlowDown (503), no 502, see cloudproxy-design.md §5.1); local `local`/`cached` objects are entirely unaffected |
...
([cli.md §2.4](../../usage/cli.md)): `forget` drops the ledger entry only; `purge` targets
...
([monitoring.md](../../usage/monitoring.md) "Tiered watermark", [storage/tiered.md](../../../architecture/storage/tiered.md) metric table).
