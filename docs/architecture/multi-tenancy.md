...
- 会话表已不是单实例内存态：STS 会话写穿 `.sys/sts/`，经 `auth.sync_interval`
  与验签前的按需回源跨实例共享（[s3-protocol.md §3.5](s3-protocol.md)），本地表只是缓存。
...
`auth.tls_identity: off|subject-cn|san-uri` 见 [tls.md §2.1](../usage/tls.md)。
...
`lights3-ctl cred create --tenant= --role=`，详见 [cli.md §3.6–§3.8](../usage/cli.md)；证书绑定
`lights3-ctl cred bind-cert|unbind-cert|list-certs`（[cli.md §3.2](../usage/cli.md)）。
