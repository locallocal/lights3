...
| `tc` / `tce` / `tcz` | HASH / HASH / ZSET（score 恒 0） | `kv_hash_key` / `kv_etag_key` / `kv_index_key` | KV 门面（S3 Tables 目录后备，§4.1）：`tc` field=key value=不透明值；`tce` 同 field 存该值的 ETag（`meta_store.h:kv_etag`，单独成 field 供脚本 `eq` 比对）；`tcz` 词序索引供 `kv_scan` 走 `ZRANGEBYLEX` |
| `inv` | pub/sub 频道 | `invalidation_channel` | 跨网关元数据缓存失效推送（§8.1）：提交脚本内 `PUBLISH`，消息 `<origin>\0<bucket>\0<key>` |
...
op 类型：`hset / hdel / hincr(HINCRBY) / zadd / zrem / del / set / pub(PUBLISH)`——
`pub` 让失效推送（§8.1）与写在同一原子执行内落地，不多一个 RTT。
...
check，`hset/hdel/hincr/zadd/zrem/del/publish` 追加 op，`commit()` 即一次
...
`put_object` / `delete_object` / `complete_upload` / `swap_extents` 四个改写对象记录的
提交在 batch 末尾追加 `publish(inv, <origin>\0<b>\0<k>)`（§8.1）。
...
### 4.1 KV 门面（S3 Tables 目录后备）

`IMetaStore::kv_get / kv_put / kv_delete / kv_scan / kv_put_batch`
（docs/architecture/s3-tables-design.md §12；契约见 rocksdb 文档 §6.1）落在
`tc`/`tce`/`tcz` 三键（§1）。写侧复用 guarded-commit 的 CAS 循环：`kv_put_batch`
逐项 `HGET tce` 取现 ETag → C++ 侧 `meta_store.h:check_kv_condition` → 脚本内对
`tce` 同 field `expect_eq(现 ETag)` / `expect_absent` 复查 → op `hset tc` +
`hset tce` + `zadd tcz`，整批一次 EVALSHA，返回 0 即重试（上限 `kMaxCasRetries`）；
`kv_put` 是单项批；`kv_delete` 先读 `tce`（缺即 false）再 `expect_eq` + 三键
`hdel/hdel/zrem`。读侧：`kv_get` = `HGET tc`；`kv_scan` = `ZRANGEBYLEX tcz`
（`after` 空取 `[prefix`，否则 `(after`，`LIMIT 0 limit`，limit=0 取 1000）取 key，
越出 prefix 即断，再 `HMGET tc` 取值，索引与取值之间被删的 key 直接略过。
...
`pool_mu_`（连接池）两把资源锁，无业务锁；唯一的常驻线程是 §8.1 的订阅线程
`sub_thread_`（`close()` 置 `sub_stop_` 后在 `pool_mu_` 外 join）。

### 8.1 失效推送：`subscribe_invalidations`

共享 redis 的多网关各自持有 `DuoStoreBackend` 的元数据缓存（core §7.1），本引擎是
四个引擎中唯一实现 `meta_store.h:IMetaStore::subscribe_invalidations`（接口默认返回
false = 无推送，缓存只靠 TTL 失效）的：

- **发布侧零成本**：§4 列出的四个改写对象记录的提交在同一 `kCommitScript` 内追加
  `pub` op，`PUBLISH <prefix>inv <origin>\0<bucket>\0<key>`——与写原子、无额外 RTT；
  `origin_` 是本 store 构造时生成的随机 id，订阅方据此跳过自己的提交（写路径已精确
  失效，晚到的自消息只会驱逐刚回填的条目）。
- **订阅侧**（`redis_meta_store.cc:RedisMetaStore::subscriber_loop`，`sub_thread_`）：
  独立 `make_conn()` 连接 `SUBSCRIBE`，命令超时改为 0 后用 `poll()`（250ms）驱动
  `redisBufferRead`/`redisGetReplyFromReader`，`sub_stop_` 标志有界退出；每条
  `message` 拆两个 `\0` 后回调 `on_key(bucket, key)`。任何错误丢弃连接、
  200ms 起指数退避（上限 5s）重连；**每次（重）订阅成功先调 `on_reset`**
  （调用方清空整个缓存）再投递消息——pub/sub 即发即弃，离线期间的失效无法补发，
  保守清空。一个 store 只允许订阅一次，重复调用返回 false。
- 接线：`DuoStoreBackend::wire_cache_invalidation` 在缓存启用且配置
  `meta_cache_feed: true`（默认）时订阅；`false` 即退回 TTL-only 陈旧语义
  （`config/lights3.yaml` duostore 段）。
...
`lights3_duostore_redis_invalidation_subscribes_total`（§8.1 订阅线程每次（重）订阅
成功 +1，每次都伴随一次缓存清空）。
