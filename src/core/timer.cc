...
TimerQueue::Id TimerQueue::add(Clock::duration delay, std::function<void()> fn) {
    ...
    {
        std::lock_guard lk(m_);
        // Refuse after shutdown: previously this returned a
        ...
        // to go back to sleep until the original deadline (previously every add did notify_all)
...
void TimerQueue::fire_loop() {
    ...
    for (;;) {
        ...
        // Duration accounting: callbacks are serial, so a slow
