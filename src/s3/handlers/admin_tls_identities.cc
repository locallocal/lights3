...
const char* mode_name(S3Service::TlsIdentityMode m) {
    switch (m) {
        case S3Service::TlsIdentityMode::Off:
            return "off";
        case S3Service::TlsIdentityMode::SubjectCn:
            return "subject-cn";
        case S3Service::TlsIdentityMode::SanUri:
            return "san-uri";
    }
    return "off";
}
...
Task<http::HttpResponse> S3Service::admin_tls_identities(http::HttpRequest& req, std::string& access_key,
    ...
    try {
        ...
        if (!tls_store_)
            throw S3Error(S3ErrorCode::InvalidRequest, "Certificate bindings are not available on this deployment.");
        ...
        if (subject.empty())
            throw S3Error(S3ErrorCode::InvalidRequest, "Usage: /-/admin/tls-identities/<subject> (or ?subject=).");
        if (!valid_tls_subject(subject))
            throw S3Error(S3ErrorCode::InvalidRequest, "Subject must be 1-" + std::to_string(kMaxTlsSubjectLen) +
        ...
        if (req.method == "PUT") {
            ...
            for (auto& [k, v] : body.items()) {
                if (k == "access_key") {
                    if (!v.is_string()) throw S3Error(S3ErrorCode::InvalidRequest, "access_key must be a string.");
            ...
            if (b.access_key.empty()) throw S3Error(S3ErrorCode::InvalidRequest, "access_key is required.");
            ...
            if (b.access_key.rfind(kSessionAkPrefix, 0) == 0)
                throw S3Error(S3ErrorCode::InvalidRequest, "STS session credentials cannot be bound to a certificate.");
            ...
            LOG_INFO("admin: certificate subject '{}' bound to {} by {}", subject, b.access_key, access_key);
