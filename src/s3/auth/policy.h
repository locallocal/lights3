...
// Standalone header: the signature layer (sigv4.h) must carry a policy snapshot at verify time, while
// credential_store.h depends on sigv4.h -- moving this back would create a cycle.
#pragma once
...
// Action granularity: previously the only switch was readonly, so "can write" implied
