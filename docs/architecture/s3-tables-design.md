...
> 状态：**已实现（§14 ①–⑥，2026-09-12）**。各步的实现记录摘要见 §16（实施稿 `docs/s3-tables/` 已随 ①–⑥ 收口删除，历史在 git）；偏离设计的点已回写到对应章节，收尾项与长期项在 [todo.md](../development/todo.md) §2 / §4 / §5。本文先回答"RustFS 是怎么做 S3 Tables
> 的"（§2，源码核实 @853ae63，2026-09-11），再给出 lights3 的方案（§3–§13）与
> 实施拆分（§14）。代码落地后，本文按仓库惯例保留为设计层文档，实现细节写进
> 对应实现文档；源码注释用 `docs/architecture/s3-tables-design.md §N` 引用本文。
>
...
[deployment.md](../usage/deployment.md) §5（多网关支持矩阵）、[cli.md](../usage/cli.md)（AdminJobs 与 `lights3-ctl`）。
...
`PUT /iceberg/v1/buckets/{bucket}` 与 CLI `lights3-ctl tables enable <bucket>`（后者
即调用前者）两个入口同一实现；管理面 `/-/admin/tables/…` 只承载 §9 的每表作业。启用前提：桶存在、
...
struct TableEntry {
    ...
    std::string version_token;       // "t-" + 32 字符 hex（getentropy 16 字节）；每次提交换新
...
进程内再加一层 per-(bucket, table_id) 的 `AsyncSemaphore(1)` 作为快路径：同一网关上的并发
...
按 [deployment.md §5.1](../usage/deployment.md) 的矩阵：duostore（redis / tikv meta +
...
redis / tikv 三处接入。部署矩阵见 [deployment.md §5.1](../usage/deployment.md)。
...
namespace 的读类操作按 `prefix_may_contain(<ns-path>/)` 判定（限在 `sales/orders/` 的
凭证仍能列出 `sales`，与 ListObjects 的 CommonPrefixes 规则一致），写类操作要求前缀本身
被允许；表类操作同时接受 `<ns-path>/<t>` 与 `<ns-path>/<t>/` 两种 key，下发的会话凭证
（前缀 `<ns-path>/<t>/`）因此能 LoadTable。于是 `prefixes: ["sales/"]` 的凭证只能操作
namespace `sales`（及子级）与其数据对象；`readonly: true` 的凭证只能 load / list / 读数据。租户凭证只见本
...
| POST `/{w}/namespaces/{ns}/tables/{t}/metrics` | ReportMetrics：解析后记审计 `tables.metrics`，回 204（§16 ⑥） | ⑥ |
...
`stage-create`、`overwrite: true` 的 register、format-version 3、无作业框架时的
`purgeRequested=true`。未实现的路径（`/transactions/commit`、`/plan`、`/tasks`、
`/sign`、`/oauth/tokens`…）没有路由，回 404 `NoSuchResourceException`；已知路径
上的错误方法回 405 `MethodNotAllowedException`。
...
- **LoadTable**：`?snapshots=all|refs`（`refs` 裁剪到 refs 与 current 引用的
  ...
  提示：`{"s3.path-style-access":"true","lights3.credential-vending":"disabled|supported",
  ...
  （`s3.endpoint` / `s3.region` 不给：网关不知道自己的对外地址，客户端本就要配）。
...
`check_range`；全部 **restart-only**（进 `docs/usage/config-reload.md §4` 表与
`app.cc` 的 `requires_restart` 比对）。`--check-config` 校验：前缀以 `/` 开头
且不含 `/v1`、`reserved_prefix` 以 `/` 结尾且不以 `.sys` 开头、多网关误配 WARN（§5.5）。

## 11. 构建接入与依赖

- 源码：`src/tables/{catalog_store.h, object_catalog_store.cc, duo_meta_catalog_store.cc,
...
- CLI：`lights3-ctl tables enable|disable|status <bucket>`、`tables list <bucket>`、
  `tables config|plan|run|purge`、`tables diagnose|recover [--prune]`；`lights3 fsck` 增加"目录状态与表桶对账"检查项
...
在两种后备间迁移（[cli.md §2.6](../usage/cli.md)）。
...
| 多表事务 `/transactions/commit` | 需要跨对象原子性，对象 CAS 做不到；duostore-meta 后备（§12）已随 ⑥ 落地、原子性条件已满足，作为长期项记 [todo.md §5](../development/todo.md) |
...
[todo.md](../development/todo.md) §2 / §4 / §5。
