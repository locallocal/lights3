...
> English translation of [../../storage/duostore-data-rados-design.md](../../../architecture/storage/duostore-data-rados-design.md). The Chinese original is authoritative; section numbering matches.
...
- ~~No distributed pin/lease for a multi-gateway data plane~~ — the read / write
  leases and the GC lease are implemented (§8.3, main document §11 `read_lease`);
  `gc_enabled` is now a deployment convention.
...
- **pool** (`rados_pool`, required): the boundary of physical placement
  ...
  remove / stat, or write_op write_full + setxattr `lights3.owner` as one atomic
  op when an owner is set; EC does not need the overwrite feature enabled, §4.1);
...
- construction maps [first,last] to the starting run/extent, O(#runs),
  algorithm unchanged;
- `read(buf)`: `rados_aio_read(ioctx, oid, buf, min(buf.size, extent remainder), extent cursor)`
  (synchronous `rados_read` in C1, AIO since C3, §6.2); when an extent is exhausted, advance to the next
...
so the pin table is the read side's only line of defense (the write side has
its own in-flight write pins, `RadosDataOptions.pins`) — under
...
The lease-based scheme has since landed along these lines (both sides in
duostore-core.md §8.5) — not a
...
closed; the deployment checklist is in [../deployment.md §5](../../usage/deployment.md).
...
used in this document (create2 / conf / write_full / aio_read / remove / stat /
nobjects_list / set_namespace / write_op setxattr) has been available since Luminous (v12,
...
1. **Obtaining a real cluster** (much heavier than redis — stated honestly):
   ...
   **this machine (no sudo, no librados) always SKIPs**. Probe protocol: run
   ...
   printed reason (not a failure; same mechanism as redis-meta §9.2). A containerized
   single-mon single-osd (`docker compose --profile rados`) fills the coverage;
   there is no CI;
