...
Part     { string data; string etag /*分片MD5 hex*/; time_point uploaded;
           string checksum_algorithm, checksum_value /*已验证片校验和，空=未声明*/; }
```
...
3. 填 meta（key/size/etag/last_modified），`backend.h:finalize_checksum(meta)`
   把 trailer 形式的校验和从 `checksum_pending` 落进 `checksum_value`
   （body 读尽后才存在），把 data move 进 `make_shared<const string>`。
...
覆写 `set_object_tagging`（基类默认 501）：锁内查到对象后直接改 `meta.tagging`，
不存在抛 `NoSuchKey`。元数据不与数据块绑定提交，所以原地改是零成本的。
另有测试钩子 `set_mtime_for_tests(bucket, key, t)` 回拨 `last_modified`
（按年龄决策的用例，如 tables 维护安全窗）。
...
- **upload_part**：先短锁校验 upload 存在（fail early），**锁外**读 body（同 PUT，
  ...
  的幽灵分片。带 `checksum` 时在 body 读尽后把 `checksum->resolved()` 与算法名
  一起存进 `Part`（trailer 形式的值此时才有）。
- **complete**：`multipart.h:validate_part_order` 预检（空/非严格递增分别抛
  ...
  与 S3 同规则）；同时把各片大小按序记进 `meta.part_sizes`（GET `?partNumber`
  布局），并以各片存储的 `checksum_algorithm/checksum_value` 调
  `multipart.h:apply_composite_checksum(digests, meta, result)`——每片都带同一算法
  的已验证值时填 COMPOSITE 校验和进 meta 与 `PutResult`；记账顺序见 §5；最后 erase
  upload。注意 5MiB 最小分片是 L2 的协议检查，存储层不设限（直接调后端 API 的
  一致性套件不受约束）。
...
（含 S3 规则的合成 ETag、`part_sizes` 布局、COMPOSITE 校验和、两个 multipart
列举的真实分页）、对象校验和持久化与回显（header/trailer 两种形式）、
`?tagging` 原地改写（§4）、`.sys` 保留桶（`kAllowReserved`，供 CredentialStore
使用）、错误一律抛 `s3::S3Error`。

不覆写的默认路径：`copy_object_fast` / `resolve_object_part` / `inspect_object`
均取基类 `nullopt`（GET `?partNumber` 由 L2 按 `part_sizes` 定位；无内部布局可
展示）。
...
- 其余以它为夹具的 L2/应用层用例：`test_reload.cc`（热重载）、`test_admin_jobs.cc` /
  `test_admin_inspect.cc`（管理面）、`test_ratelimit.cc`、`test_access_log.cc`、
  `test_api_metrics.cc`、`test_tenancy.cc`、`test_tls_identity.cc`、
  `test_tables_*.cc`（S3 Tables，`set_mtime_for_tests` 的用户）。
