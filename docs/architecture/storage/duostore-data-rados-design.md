...
- ~~多网关数据面的分布式 pin/租约不实现~~——读写租约与 GC 租约已实现
  （§8.3，主文档 §11 `read_lease`）；`gc_enabled` 退为部署约定。
...
| Extent 字段 | kRados 语义 |
| --- | --- |
| file_id | rados 对象名 = `c.<file_id:016x>`；`IMetaStore::alloc_file_run(kRados, n)` 连续号段分配（`FileIdAlloc`），号段机制照搬（主文档 §4.5）。实现上 kRados 与 kChunk **共号段**：refs 表按裸 file_id 记账不分 kind，独立计数器会在同一 meta 上切换 data 引擎（fs↔rados）时产生跨 kind id 碰撞 |
...
- **pool**（`rados_pool`，必填）：物理放置策略的边界——副本数、EC
  ...
  stat，带 owner 时为 write_op write_full + setxattr `lights3.owner` 原子提交；EC 不需开
  overwrite 特性，§4.1）；
...
- `read(buf)`：`rados_aio_read(ioctx, oid, buf, min(buf.size, extent 剩余), extent 游标)`
  （C1 为同步 `rados_read`，C3 起 AIO，§6.2）；extent 读尽推进到下一个对象。无 fd 概念——fs 版
...
"已打开 fd 不受 unlink 影响"兜底，pin 表是读侧唯一防线（写侧另有在途写 pin，`RadosDataOptions.pins`）——单网关
部署下进程内计数即完全正确（与主文档同一论证）。
...
| 读侧 pin vs 他网关 GC | **已补**：pin 表虽为进程内，但各网关经 `read_lease`（默认 5s）向共享 meta 发布"最老在途读开始时间"，GC 网关只回收所有对端在途读都晚于其入队的 gcq 项/见空早于下限的空 pack（[storage/duostore-core.md §8.5](duostore-core.md)）；`read_lease: 0` 关闭时回落旧约束 `gc_grace` ≥ 最长预期 GET 时长 |
| 写侧 pin vs 他网关孤儿扫描 | **已补**：在途 PUT / 分片已落盘未入 refs 的 chunk 只有进程内写侧 pin 保护，超过 `gc_grace` 的长上传会被对端孤儿扫描当作崩溃遗留删除；现在同一条租约携带"最老在途写开始时间"，孤儿扫描只删 mtime 早于该下限（减偏差余量）的无 refs chunk（[storage/duostore-core.md §8.5](duostore-core.md)） |
| multipart 跨网关 | **已验证**：create / upload_part / complete / abort 可落在不同网关——upload_id 与 file_id 全局唯一、四步各为单事务、complete 纯 meta 装配；两个 `DuoStoreBackend` 共享 meta + 数据面对象的单测套件（`tests/unit/multi_gateway_suite.h`，redis / tikv 各 5 例）、`run_e2e.sh` 双网关段与 compose `multi` profile 覆盖 |
| GC/孤儿扫描的执行者 | **已补 GC 租约**：每轮 GC / 孤儿扫描 / mpu 清理先 `try_gc_lease`（redis/tikv 上原子 CAS+TTL，TTL = max(2×gc_interval, 600s)），抢不到即跳过本轮；多实例同时开 `gc_enabled` 不再互踩 |

部署约定（C4 起有配置承载）：**多网关时建议 GC 只由指定的单一实例执行**（GC 租约兜底，见上表）；
...
具体条目并已关闭；部署清单见 [../deployment.md §5](../../usage/deployment.md)。
...
（create2 / conf / write_full / aio_read / remove / stat / nobjects_list /
set_namespace / write_op setxattr）Luminous（v12，2017）起全部可用，不做版本特判。
...
1. **真实集群的获取**（比 redis 重得多，如实声明）：redis-server 是单
   ...
   否则显式 SKIP 并打印原因（不算失败，机制同 redis-meta §9.2）。容器化
   单 mon 单 osd（`docker compose --profile rados`）补覆盖，无 CI；
