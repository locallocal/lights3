...
> English translation of [../s3-protocol.md](../../architecture/s3-protocol.md). The Chinese original is authoritative; section numbering matches.
...
- **Parsing**: request XML is parsed only for CompleteMultipartUpload,
  DeleteObjects, CreateBucket (LocationConstraint) and the PUT bodies of
  `?cors` / `?lifecycle` / `?website` / `?quota` / `?tagging`. All structures are shallow;
...
- **Conditional requests**: GET/HEAD/PUT support If-Match / If-None-Match;
  ...
  **process-local**, and multi-instance deployments sharing one filesystem are
  out of scope ([architecture.md](overview.md) non-goal: the data plane
...
- **Metrics** (Prometheus text format, `GET /-/metrics`, on the data-plane
  listener by default; with `http.admin_port` set it is served only on the
  separate admin listener and the data-plane port answers 404
  ([http-adapter.md §2.1](http-adapter.md)); `http.metrics_access: anonymous` (default, the classic scrape) or
