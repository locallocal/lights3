...
**without threading the token through 40+ signatures**. The
...
idle-blocked for the whole request. `sync_wait_pumping`
instead:
...
- **In `background.cc::run_detached` the task frame must be destroyed before
  ...
  moved into a local of an inner scope first.
...
3. **Resuming in place on the cancel path is deliberate**: the resumed
   ...
   exactly the scenario that needs it most — a saturated pool; a strict
   "always post back to the pool" rule deadlocks here (see the corresponding
