...
std::optional<SysTime> parse_http_date(const std::string& s) {
    ...
    if (sscanf(s.c_str(), "%*3s, %2d %3s %4d %2d:%2d:%2d", &tm.tm_mday, mon, &tm.tm_year, &tm.tm_hour, &tm.tm_min,
               &tm.tm_sec) != 6)
        return std::nullopt;
    tm.tm_mon = -1;
    for (int i = 0; i < 12; ++i)
...
std::string iso8601(SysTime t) {
    auto tm = to_utc_tm(t);
    char buf[48];
    snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02d.000Z", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
...
std::string amz_date(SysTime t) {
    auto tm = to_utc_tm(t);
    char buf[40];
    snprintf(buf, sizeof(buf), "%04d%02d%02dT%02d%02d%02dZ", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
             tm.tm_min, tm.tm_sec);
    return buf;
}
...
std::optional<SysTime> parse_amz_date(const std::string& s) {
    std::tm tm{};
    // Fixed format "YYYYMMDDTHHMMSSZ" (16 chars): %n verifies the 'Z' is present, the length check rejects trailing
    // garbage
    int consumed = 0;
    if (sscanf(s.c_str(), "%4d%2d%2dT%2d%2d%2dZ%n", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min,
               &tm.tm_sec, &consumed) != 6 ||
