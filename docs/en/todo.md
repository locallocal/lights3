...
| S3 Tables (Iceberg REST Catalog) | Design [s3-tables-design.md](s3-tables-design.md), implementation documents under `docs/s3-tables/`; **①–④ implemented (2026-09-12)**: catalog, table buckets, commit protocol, guard, policy / tenant gate, the `s3tables` signing name, credential vending, lifecycle exclusion, deep Avro validation, `catalog/diagnostics` / `recovery`, rename recovery, `fsck` reconciliation, LoadTable ETag, maintenance plan / run / purge with the periodic runner, `lights3-ctl tables`, unit tests and an e2e segment. Next ⑤ multi-gateway and docs |
