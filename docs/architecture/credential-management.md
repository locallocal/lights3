...
> 状态：一期、二期均已实现（分期见 §9，二期设计见 §10）。承接 docs/architecture/s3-protocol.md §3.5 预留的 `ICredentialProvider` 扩展点。
...
- STS 临时凭证 / 凭证轮换到期——STS 会话凭证（`AssumeRole`）已在后续落地（§9.1、
  [s3-protocol.md §3.5](s3-protocol.md)）；永久凭证的轮换到期仍未做；
...
子命令框架为 `third_party/ccmd`）。凭证操作归于 `cred` 命令组：`cred list` / `cred get <ak>` /
`cred create` / `cred delete <ak>` 对应上表的 GET / GET / POST / DELETE（`PUT`
就地修改暂无 CLI 包装），另有证书绑定 `cred bind-cert` / `cred unbind-cert` /
`cred list-certs`（[tls.md §2.1](../usage/tls.md)）；均 SigV4 自签名。root AK/SK 经 `--ak=`/`--sk=` 或环境变量
`LIGHTS3_ADMIN_AK`/`LIGHTS3_ADMIN_SK` 传入（SK 建议走环境变量，argv 对本机
`ps` 可见）；选项须写在叶子子命令之后，长选项取值 `--name=value` 与 `--name value`
两种形式均可（ccmd v0.0.2）；`cred get` 支持 `--show-secret`，`cred create` 支持
`--comment` 与 `--policy`（内联 JSON 或 `@file`）。完整命令参考见
[cli.md §3.2](../usage/cli.md)，或 `lights3-ctl help cred [command]`。
...
- 凭证写入 `default_backend` 上名为 `.sys` 的 bucket，对象键
  ...
  （主体百分号编码为单一路径段，[tls.md §2.1](../usage/tls.md)）；
...
把私有的 `std::map creds_` 换成 docs/architecture/s3-protocol.md §3.5 预留的接口：
...
- 多实例限制：多个网关实例共享同一后端时，实例间无失效/新增通知，
  ...
  reload（§10.3）；不开启时仍是单进程假设（docs/architecture/overview.md 的部署模型）。
...
- 每轮：先采内存中动态凭证的 AK 快照，再 list `.sys/credentials/`——
  ...
  不会被误删；
- 已存在的 AK 按存储 ETag 对比决定是否重拉：`PUT /-/admin/credentials/{ak}`（§2）
  就地修改 policy/comment 并把 `rev` +1，对端在下一轮 sync 看到 ETag 变化即重读；
  SK 本身仍不可变；
...
- 携带方式：`POST /-/admin/credentials` 的 JSON body
  ...
  credentials_file 条目的 `policy` 字段；创建后可经 `PUT /-/admin/credentials/{ak}`
  就地修改（§2，`"policy": null` 清除）；
