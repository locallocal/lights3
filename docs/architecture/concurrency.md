...
| `core/executor.h` | `IExecutor` 投递抽象、`InlineExecutor`、`PumpExecutor`、`resume_on` |
...
2. **ThreadPoolExecutor**：`post` = 把 resume 包成任务无界入队线程池。
   `Application`（`src/app/app.cc`）用它做信号量唤醒的投递目标（§6）；
3. **PumpExecutor**：同步驱动的连接/请求线程在 `sync_wait_pumping` 里兼职泵队列，
   `post` 投递到该线程（[coroutine-internals.md §2.2](coroutine-internals.md)）；
4. `co_await resume_on(ex)`：把当前协程的后续执行切到指定 executor，
   业务代码需要明确落点时使用。
...
- `join()`：停止接收新任务，**排空队列**（含 backlog）后等待线程退出；
  join 后 `schedule` 抛异常，`post` 不抛（消费者多为 noexcept 路径）——记 ERROR
  并在调用线程就地执行。
...
thread-per-request：请求线程 `resp = sync_wait_pumping(exec, handler_(std::move(req)))`，
等待期间兼职 `PumpExecutor` 泵队列——body 的阻塞读经 `co_await resume_on(exec)`
切回请求线程执行，慢客户端只拖垮自己的线程，不占共享池；其余子任务在池线程
完成后就地推进续体到下一个挂起点或结束，顶层完成时 `exec.finish()` 结束泵循环。

流式响应同理：builtin 的整个响应循环是一个 `sync_wait_pumping` 驱动的协程，
httplib 的 content provider 每块经 `StreamPrefetch::next_sync()` 收取（两者都
双缓冲预取，[http-adapter.md §2.4](http-adapter.md) ①⑤）。
...
进程级单例、双线程的定时器：调度线程只判到期，专职回调线程串行执行回调
（[coroutine-internals.md §7](coroutine-internals.md)）。`add(delay, fn)` 返回 id，
`cancel(id)` 撤销未触发的定时器（已到期未执行完则阻塞等它结束）。回调在回调
线程上串行执行，**必须轻量**——典型只做 `request_cancel()` 或 executor post。
...
| 位置 | 用途 |
| --- | --- |
| dispatch 入口（`http/admission.h`，由 `src/app/app.cc` 接线） | `runtime.max_inflight_requests` 全局限流，超限请求排队而非拒绝；等待者经池 executor 唤醒。流式响应的 Permit 系在 `stream_body` 上（最外层包装），驱动读完/断连丢弃时才归还——限流覆盖响应传输全程，而非只覆盖 handler 协程帧；关停排空按 `available()` 判在途，因此也数得到流式传输中的请求 |
...
只维护一份），现由 duostore、tiered、localfs、CredentialStore
（credential-management.md §10.3）以及 website / usage / lifecycle /
sys_config 各 store、scrub 限速器与 tables 维护器共用。解决的问题：定时器驱动的
