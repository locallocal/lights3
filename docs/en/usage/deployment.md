...
The build-and-distribution write-up. Five things: the **version / build identity** inside
...
docker run --rm -e LIGHTS3_SECRET_1=x lights3 --version   # a leading - flag takes the server path; the entrypoint exits 2 without LIGHTS3_SECRET_1
...
| `LIGHTS3_TIKV` | `OFF` | `libgrpc++-dev protobuf-compiler-grpc libprotobuf-dev libpoco-dev libabsl-dev` ([duostore-meta-tikv-design.md §8](../architecture/storage/duostore-meta-tikv-design.md)) |
...
docker compose --profile multi run --rm e2e-multi   # a 5-part multipart spread over both gateways
```
...
([testing.md §1](../development/testing.md)). The `e2e` compose profile brings all three
...
its `.sys`, [s3-tables-design.md §5.5](../architecture/s3-tables-design.md)); a table bucket itself may
...
| instance-level background jobs (`usage.reconcile`, tiered scans, …) | on one instance only | the same designated-instance semantics as `gc_enabled` ([multi-tenancy.md](../architecture/multi-tenancy.md)) |
...
**Table catalog across gateways** ([s3-tables-design.md §5.5](../architecture/s3-tables-design.md)):
...
sudo ./scripts/rollback.sh           # swap *.prev back and restart; running it again swaps forward (--no-restart: swap only)
...
- **install.sh** validates the live `/etc/lights3/lights3.yaml` with the
  **new** binary's `--check-config` before touching anything (sourcing
  ...
  start-stop are delegated to `lights3-setup.sh` (§3.3), which install.sh
  places at `/usr/local/share/lights3/lights3-setup.sh`.
- **rollback.sh**: runs `--check-config` with `lights3.prev` first (an older
  ...
  renames and restarts the service if it is running (`--no-restart` skips the
  restart). Config and data are left alone.
- **uninstall.sh**: `remove` → delete files (including `/usr/local/share/lights3`)
  → `daemon-reload`; `--purge` adds the helper's `purge`. Package installs are removed with the package manager
  (§3.1).
