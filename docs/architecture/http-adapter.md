...
- TLS：连接线程上以 OpenSSL 阻塞 I/O 包裹 socket（`Io` 抽象统一 recv/send），
  证书/SNI/热重载来自共享的 `tls::Holder`（[tls.md](../usage/tls.md) §3）。
...
- 适配方式：在 httplib 的 handler 回调里 `sync_wait_pumping(req_exec, handler(req))`——
  当前线程边等边兼职 `PumpExecutor`（body 的阻塞读切回本线程）。协程内部切换到
  IO 线程池的 `co_await` 依旧成立，完成后 resume 发生在池线程，顶层完成时
  `finish()` 结束泵循环。
