// L3: xlocalfs -- io_uring data-plane variant of localfs (see docs/architecture/storage/storage-backend.md §3.3).
...
// writes), the commit-phase fdatasync rides as a linked SQE behind the
...
public:
    ...
    // UNLINKAT via the ring when the kernel has it: unlinking a large
...
private:
    ...
    // outcome of the caller's set_meta_xattr (drives the sidecar policy)
