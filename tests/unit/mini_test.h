...
void check_eq(const A& a, const B& b, const char* ea, const char* eb, const char* file, int line) {
    if (!(a == b)) {
        ...
        os << file << ":" << line << " CHECK_EQ(" << ea << ", " << eb << ") failed: '" << a << "' != '" << b << "'";
...
// The test process restarts itself with arguments (execv /proc/self/exe <mode> ...) to enter a registered child
// routine; the child routine ends with _exit / being SIGKILLed to simulate kill -9; the parent reopens the state
// directory and verifies convergence
...
#define CHECK_THROWS_S3(expr, expected_code)                                                      \
    do {                                                                                          \
        ...
        try {                                                                                     \
            ...
            thrown_ = true;                                                                       \
            CHECK_EQ(::lights3::s3::wire_code(e_.code), ::lights3::s3::wire_code(expected_code)); \
        }                                                                                         \
        if (!thrown_) {                                                                           \
            std::ostringstream os_;                                                               \
