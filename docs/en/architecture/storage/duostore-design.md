...
> English translation of [../../storage/duostore-design.md](../../../architecture/storage/duostore-design.md). The Chinese original is authoritative; section numbering matches.
...
struct IMetaStore {
    ...
    virtual std::vector<UploadInfo> list_uploads(std::string_view b, std::string_view key_marker,
                                                 std::string_view id_marker, size_t limit,
    ...
    virtual uint64_t alloc_file_run(Extent::Kind kind, uint32_t n) = 0;  // durably monotonic, contiguous run (≤64); alloc_file_id is the inline n=1 wrapper
    virtual std::vector<std::pair<uint64_t, Reclaim>> peek_reclaims(size_t max, uint64_t min_seq,
...
(The sketch is abbreviated; `meta_store.h` is authoritative. Omitted:
`head_object`, the `PutCondition` parameter of `put_object`, the pack ledger
`seal_pack` / `drop_pack_stat` / `ack_reclaims`, compaction
`swap_extents_batch` / `scan_refs`, the multi-gateway leases `try_gc_lease` /
`publish_lease` / `min_lease`, `snapshot()`, the S3 Tables `kv_*` facade and
the backup / restore methods; see
[duostore-core.md](../../../architecture/storage/duostore-core.md) §4.)
...
struct WriteHint {
    ...
    std::string owner;                       // ownership embedded in the pack record ("bucket\0key" / "mpu\0<id>\0<part_no>", §5.2)
};
...
(The sketch is abbreviated; `data_store.h` is authoritative. Omitted:
`write_batch`, `remove_pack`, `seal_aged_packs`, `scan_chunks` / `scan_packs`
(orphan-scan enumeration), `pack_write_locked`, `stat_pack`; `Extent::Kind`
also has `kRados = 2`.)
...
| `uploads` | `<bucket>\0<key>\0<upload_id>` | `{v2, initiated_ms, content_type, user_meta, first-class metadata}` | a prefix scan of `<bucket>\0` is naturally sorted by (key, upload_id) |
| `parts` | `<bucket>\0<key>\0<id>\0<be16 part_no>` | `{v2, size, md5, modified_ms, extent_runs, checksum_algorithm/checksum_value}` | big-endian part_no guarantees ascending order; list_parts is just a prefix scan |
...
| `stats` | `p<be64 pack_id><b\|r\|s>` / `c0` `c1` `q` | merge counters | pack liveness ledger (`b`/`r` = incremental merge of live_bytes/live_recs, `s` = sealed flag); `c0`/`c1` = chunk/pack file_id segments, `q` = gcq seq segment |
| `tc` | S3 Tables key | opaque | KV facade for the S3 Tables catalog (`kv_*`) |
...
`Task<DuoGcStats> run_gc_once()`** (called directly by tests, following the
tiered P1 "manual sink trigger" precedent); the orphan scan is independent and
low-frequency (`orphan_scan_interval` default 1d) + the same style of hook
`run_orphan_scan_once()`; a read-only integrity hook `run_scrub_once()` also exists. GC
...
- pack extent → the liveness ledger was already decremented in the business
  ...
  `live_recs == 0` and sealed → once it has also been empty for `gc_grace`
  (`pack_empty_since_` cooldown), is unpinned and predates every peer read
  lease → unlink the whole file + delete its packstat.
...
- the reverse direction: refs present but file missing → **alert counter,
  ...
  intervention);
- an unaccounted record inside a pack is dead space, naturally reclaimed by
  compaction; the orphan scan additionally enumerates `packs/` via
  `IDataStore::scan_packs` and unlinks pack files with no packstat row that
  are past grace, unpinned and not flock-held (a crash between file creation
  and the first record commit).
...
| GC compaction hits a corrupt record | skip + alert; the remaining live records migrate as usual; a pack with corrupt records and zero migration for 3 consecutive rounds (`kQuarantineStrikes`) enters the quarantine ledger `<root>/quarantine` (file kept) for manual handling via `lights3 duostore quarantine list\|release\|purge` |
...
| pack_max_age | 1h | age-based sealing of active packs (so dead regions reach the compaction candidate set under low write volume); 0 = off |
...
| data | io_uring FsDataStore (implemented, `fs_uring: true`) | reuses xlocalfs's `UringEngine`/`uring_stream` pipeline: the data plane swaps its IO engine, layout unchanged ([duostore-data-fs.md](../../../architecture/storage/duostore-data-fs.md) §9) |
...
pin table of §7 is no longer sufficient — this is covered by the read / write
leases plus the GC lease (§11 `read_lease`,
[duostore-core.md](../../../architecture/storage/duostore-core.md) §8.5).
...
- **Can serve as tiered's local side** (roadmap §3.6 ⑥): tiered's local side
  ...
  [tiered.md](../../../architecture/storage/tiered.md) §11; accepted by
  `e2e_tiered_duolocal`);
...
(six duostore .cc files + `tier_local_duo.cc`) +
...
1. **Consistency suite**: `tests/unit/test_storage.cc` adds suite cases under
   ...
   all-pack), `chunk_size` shrunk to 4KiB (forcing many-chunk manifests); plus
   two `duostore_backend_suite_uring*` variants with `fs_uring: true` (five
   suites in total);
