...
- The session table is no longer single-instance in-memory: STS sessions are
  written through to `.sys/sts/` and shared across instances via
  `auth.sync_interval` plus the on-demand load before signature verification
  ([s3-protocol.md §3.5](s3-protocol.md)); the local table is only a cache.
...
| `GET /-/admin/tls-identities` | root | `{"mode","identities":[{subject,access_key,comment?,created_at,created_by}]}` ([tls.md §2.1](../usage/tls.md)) |
...
described in [tls.md §2.1](../usage/tls.md).
...
`lights3-ctl cred create --tenant= --role=`; see [cli.md §3.6–§3.8](../usage/cli.md); certificate
bindings via `lights3-ctl cred bind-cert|unbind-cert|list-certs` ([cli.md §3.2](../usage/cli.md)).
