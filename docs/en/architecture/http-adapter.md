...
> English translation of [../http-adapter.md](../../architecture/http-adapter.md). The Chinese original is authoritative; section numbering matches.
...
- **Separate admin port** (backlog-sequence ②): `http.admin_port` (absent = no
  ...
  `--endpoint` must point at the admin port ([cli.md §3.1](../usage/cli.md)), Prometheus
...
- TLS: OpenSSL blocking I/O wrapped around the socket on the connection thread
  ...
  the shared `tls::Holder` ([tls.md](../usage/tls.md) §3).
...
- Bridging `Task<T>` with asio: no conversion between `asio::awaitable` and
...
- Adaptation: inside httplib's handler callback,
  `sync_wait_pumping(req_exec, handler(req))` — the current thread doubles as a
  `PumpExecutor` while it waits (blocking body reads are moved back onto it).
  `co_await`s that switch to the IO thread pool inside the coroutine still hold;
  on completion the resume happens on a pool thread, and `finish()` ends the
  pump loop when the top-level task completes.
...
- TLS: every shard builds credentials with `seastar::tls::credentials_builder`
  ...
  OpenSSL backend ([tls.md](../usage/tls.md) §4).
