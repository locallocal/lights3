...
> English translation of [../object-read-write-flow.md](../../architecture/object-read-write-flow.md). The Chinese original is authoritative; section numbering matches.
...
[architecture.md](overview.md); per-layer internals are in
...
S3Service::dispatch()                       src/s3/service.cc
  ...
  │    (docs/architecture/credential-management.md §4.1)
  ...
  │    see docs/architecture/credential-management.md §10.4)
  ▼
S3Service::route()                          explicit dispatch table (docs/architecture/s3-protocol.md §2)
...
drivers (builtin/httplib) `sync_wait_pumping` on the driver thread (which
doubles as the pump executor), asynchronous drivers hang it on their own event
loop (docs/architecture/concurrency.md §4).
...
2. conditional requests (docs/architecture/s3-protocol.md §6): `If-None-Match: *` first
...
- Client disconnects are thrown upward by the driver-layer reader as exceptions
  (contract in docs/architecture/http-adapter.md §4); the whole coroutine chain unwinds and
...
- **Multipart** (details in docs/architecture/storage/storage-backend.md §3.2 and
  docs/architecture/s3-protocol.md §1): `upload_part` is fully isomorphic to PUT
...
construct FdStreamReader(fd, offset=f, remaining=len)   ← fd ownership moves into the reader
```

`FdStreamReader.read()` does `co_await pool_->schedule()` first each time, then
...
3. streaming response loop: the whole loop is one coroutine driven by
   `sync_wait_pumping` (`builtin_server.cc:stream_body`); `StreamPrefetch`
   double-buffers the next chunk while the current one is `send`-ed, until EOF;
   a fixed-length plaintext file body first tries the `sendfile(2)` fast path
   (`http.sendfile`, docs/architecture/http-adapter.md §2.4 ④⑤);
...
```text
 ...
 ───────────────                               ───────────────
 socket ──SocketBodyReader──┐                  ┌──FdStreamReader/UringStreamBodyReader── fd
