...
├── aws_credentials.h/.cc      # AwsCreds / CredentialProvider：环境变量 → 容器端点
│                              # → IMDSv2 凭证链（§2.3.1），不含 httplib 类型
...
- `cloudproxy_backend.h:CloudProxyConfig` 承载全部配置项；
  ...
  `queue_cap ∈ [4KiB,1GiB]`、`connect_timeout_ms ∈ [1,600000]`，默认 5000，
  喂 `set_connection_timeout`），排除运行期算术意外（如退避左移溢出）；
...
设置了 AWS_CONTAINER_AUTHORIZATION_TOKEN 时原样作 `Authorization` 头）→
...
1. 校验 key、映射 bucket（§6.1）、range 格式化为 `Range: bytes=...`
   进 extra 头，并恒带 `x-amz-checksum-mode: ENABLED`（远端才会回显
   `x-amz-checksum-*`，见 §6.2 head_object）。所有出站请求的 extra 头都先过
   `cloudproxy_backend.cc:CloudProxyBackend::trace_extra`：当前 await 链携带
   `RequestBackendStats`（`storage/request_stats.h`）且 trace 有效时追加
   `traceparent`（网关自身 span）与 `tracestate`；后台任务（tiered 下沉/GC）
   无请求上下文，不加；
...
全部走 `co_await retry_io(op, fn)` 协程模板（每轮 = 熔断闸 → `acquire_async`
→ 一次 `control_io` 阻塞发送，§3.1 + §7.2）：
...
| head_object | HEAD `<object_path>`（带 `x-amz-checksum-mode: ENABLED`） | 200 → `cloudproxy_backend.cc:meta_from_response`（ETag 去引号、Content-Length→size、Last-Modified、七个首等元数据头按 `backend.h:kStdMetaFields` 表回填、`x-amz-checksum-{crc32,crc32c,crc64nvme,sha1,sha256}` → `checksum_algorithm`（大写）/`checksum_value` 与 `x-amz-checksum-type` → `checksum_type`、`x-amz-meta-*` 剥前缀小写化进 user_meta）；HEAD 无错误体，404 靠 `ErrCtx::Key` 补 NoSuchKey |
| resolve_object_part | HEAD `<object_path>?partNumber=N`（op `head_part`） | GET `?partNumber` 支持：分片布局归远端所有，206 时解析 `Content-Range: bytes a-b/total` 得 offset/size，200 按整对象；`x-amz-mp-parts-count` → `parts_count`（缺省 1）；416 → `InvalidPartNumber` |
| set_object_tagging | PUT `?tagging`（op `put_tagging`）/ 空 tagging 时 DELETE `?tagging`（op `delete_tagging`） | 把 L2 规范化的 `k=v&k=v` 串还原成 Tagging XML（body 精确 sha256 签名）转发，非 2xx → `ErrCtx::Key` |
...
PUT，与其余控制面同样重试（§7.2）。响应处理与 complete 同坑：**慢 COPY
...
- `create_multipart`：POST `?uploads`（meta 头此时上，content_type 走
  ...
  注意 create 走 `retry_io`，响应丢失后的重试可能在远端留下空孤儿
  upload——已知取舍，建议远端配 AbortIncompleteMultipartUpload 生命周期规则；
- `upload_part(…, body, const std::optional<PartChecksum>& checksum)`：PUT
  `?partNumber=N&uploadId=`，直接进 §5 的 `stream_upload(multipart_ctx=true)`，
  ETag 透传；**头形态**的分片校验和以 `x-amz-checksum-<algo 小写>` 转发让远端
  同样校验/存储，**trailer 形态**的值在头部发出后才到——本网关已校验，远端
  不获知（已知限制）；
- `complete_multipart`：见 §7.3（含 200-错误体与歧义消解）；XML 里每个
  `<Part>` 若 `PartInfo` 带 `checksum_algorithm/checksum_value` 则追加
...
head_bucket / list_buckets / head / head_part / delete / put_tagging /
delete_tagging / list / create_multipart / complete_multipart /
abort_multipart / list_parts / list_uploads / copy，数据面 get / put /
upload_part。
