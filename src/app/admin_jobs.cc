...
FsckOutcome run_scrub(storage::IStorageBackend& backend, uint64_t max_bytes_per_sec) {
    ...
    if (auto* duo = dynamic_cast<storage::DuoStoreBackend*>(&backend)) {
        ...
        out.kind = "duostore";
        out.stats = {{"objects_scanned", st.objects_scanned}, {"parts_scanned", st.parts_scanned},
                     {"extents_checked", st.extents_checked}, {"bytes_read", st.bytes_read},
                     {"corrupt_extents", st.corrupt_extents}, {"unreadable_extents", st.unreadable_extents},
                     {"objects_bad", st.objects_bad},         {"refs_missing", st.refs_missing},
                     {"refs_stale", st.refs_stale},           {"meta_errors", st.meta_errors}};
        out.findings = st.corrupt_extents + st.unreadable_extents + st.refs_missing + st.meta_errors;
    ...
    if (auto* lfs = dynamic_cast<storage::LocalFsBackend*>(&backend)) {
        ...
        out.kind = "localfs";
        out.stats = {{"objects_scanned", st.objects_scanned}, {"bytes_read", st.bytes_read},
                     {"etag_mismatches", st.etag_mismatches}, {"read_errors", st.read_errors},
                     {"unverifiable", st.unverifiable},       {"skipped_stubs", st.skipped_stubs},
                     {"skipped_races", st.skipped_races},     {"orphan_sidecars", st.orphan_sidecars}};
        out.findings = st.etag_mismatches + st.read_errors;
        out.aborted = st.aborted;
        return out;
    }
    throw std::invalid_argument("this backend type has no offline fsck (duostore, localfs and xlocalfs do)");
}
...
JobOutcome run_duo_job(JobOp op, storage::DuoStoreBackend& duo) {
    ...
    switch (op) {
        case JobOp::DuoGc: {
            ...
            out.stats = {
                {"reclaims_acked", st.reclaims_acked},      {"files_removed", st.files_removed},
                {"skipped_grace", st.skipped_grace},        {"skipped_pinned", st.skipped_pinned},
                {"skipped_leased", st.skipped_leased},      {"packs_removed", st.packs_removed},
                {"uploads_expired", st.uploads_expired},    {"packs_sealed_aged", st.packs_sealed_aged},
                {"packs_compacted", st.packs_compacted},    {"packs_compact_deferred", st.packs_compact_deferred},
                {"records_migrated", st.records_migrated},  {"records_corrupt", st.records_corrupt},
...
JobOutcome run_job(JobOp op, storage::IStorageBackend& backend, uint64_t max_bytes_per_sec) {
    switch (op) {
        case JobOp::Fsck:
            return run_scrub(backend, max_bytes_per_sec);
        case JobOp::DuoGc:
        case JobOp::DuoScan: {
#ifdef LIGHTS3_DUOSTORE
            if (auto* duo = dynamic_cast<storage::DuoStoreBackend*>(&backend)) return run_duo_job(op, *duo);
#endif
        ...
        case JobOp::Fsck:
            ...
            if (!supported)
                throw Failure{Error::Unsupported,
                              "backend '" + backend + "' has no offline fsck (duostore, localfs and xlocalfs do)"};
            break;
        case JobOp::DuoGc:
        case JobOp::DuoScan:
#ifdef LIGHTS3_DUOSTORE
...
#endif
            if (!supported) throw Failure{Error::Unsupported, "backend '" + backend + "' is not a duostore backend"};
            break;
        case JobOp::TierScan:
        case JobOp::TierGc:
        case JobOp::TierReconcile:
...
void AdminJobs::finish_job(Job& j, JobOp op, JobOutcome out, std::string error) {
    ...
    if (!j.error.empty())
        ...
        LOG_INFO("{} {} job {} finished: kind={} findings={} aborted={} ({} ms)", job_group_name(op), job_op_name(op),
