...
九个 column family（`rocks_meta_store.h:RocksMetaStore::Cf` 枚举，打开顺序即索引），
...
| `objects` | `codec.cc:object_key` = `<bucket>\0<key>` | `codec.cc:encode_object`（ObjectVal v3，§2） | 字节序 = S3 字典序，直接支撑 list（§5） |
...
| `tc` | 不透明 key（调用方自定） | 不透明 value | KV 门面（S3 Tables 目录后备，§6.1）；ETag = `meta_store.h:kv_etag` = sha256 hex 前 16 字符；存量库打开时经 `create_missing_column_families` 自动补建 |
...
- **版本容错读**（`codec.cc:read_ver`）：object 当前写 v3（v2 加一等元数据 kv 段，
  v3 加分层段 `u8 tier | str remote_etag | str remote_at`，对应 `meta_store.h:TierState`），
  upload 写 v2，part 写 v2（加 `str checksum_algorithm | str checksum_value`），读侧
  各自接受全部旧版本——升级不需要停机重写存量记录，但旧二进制读不了 v3，共享 meta
  的网关须一起升级；bucket/gcq 恒 v1（`codec.cc:check_ver` 严格相等）。
...
- `compression = kNoCompression` 恒关、不外露（主文档 §13.3：元数据体量小，换零外部
  依赖）；`create_if_missing` / `create_missing_column_families` 开启，九个 CF 一次
...
- **`snapshot()`**（在线 dump，roadmap §3.7）：返回
  ...
  体，四个读观察同一时刻状态。视图借用 store，调用方（`run_meta_dump`）必须在
  `close()` 前销毁。
- **`kv_get` / `kv_scan`**（KV 门面，§6.1）：`tc` CF 点查 / 前缀迭代，`after` 非空时
  从 `after` 起 seek 并跳过 `<= after` 的 key，`limit=0` 取 1000。
...
| `kv_put` / `kv_put_batch` | 逐项读 `tc` 现值取 `kv_etag`，`meta_store.h:check_kv_condition` 校验 `PutCondition`（任一项违反即抛，批不提交） | 全部 Put tc，一批一次 `commit()`；返回各项新 ETag（§6.1） |
| `kv_delete` | 读 `tc`，缺即返回 false（幂等，无写） | 单 Delete tc |
...
### 6.1 KV 门面（S3 Tables 目录后备）

`meta_store.h:IMetaStore::kv_get / kv_put / kv_delete / kv_scan / kv_put_batch`
（docs/architecture/s3-tables-design.md §12）是独立于对象表的不透明有序 KV 空间，
`tables.catalog_backing = duostore` 时承载 S3 Tables 目录；接口默认实现抛
NotImplemented，四个引擎都覆写。本实现落在 `tc` CF：value 原样存放，ETag 由共享的
`meta_store.h:kv_etag`（sha256 hex 前 16 字符）计算；`kv_put` 是单项的
`kv_put_batch`，后者持 `mu_` 内逐项读现值并跑 `check_kv_condition`
（`if_none_match` 撞存在项 / `if_match_etag` 不符 → PreconditionFailed，
`if_match_etag` 遇缺失 → NoSuchKey），全部通过后单 WriteBatch 一次 `commit()`
——目录的 commit 记录与指针同批落地。`kv_delete` 持 `mu_`，缺失返回 false。
