...
| `unit_tests` stack overflow under ASan | 2026-09-06 full `build-asan` run: in `test_http_drivers` the builtin `stream_body` recurses synchronously without bound through `StreamPrefetch::finish → start_read → Started::start → read` (`PatternReader` completes synchronously); with ASan's larger frames thread T2040 overflows its stack (`drivers/common.h:127–154`). Non-ASan builds and the seastar variant pass; the previous ASan record (08-14, 324 cases) predates the prefetch code | Investigate the `StreamPrefetch` recursion for synchronously completing readers: loop / trampoline instead of calling `start_read` from `finish`, or defer a synchronous completion to the next `next()` |
...
| --- | --- | --- | --- | --- |
| Large-object PUT stack overflow on the builtin driver (**crash**) | Found 2026-09-06 while extending the e2e, reproduces on main: a 32 MiB PUT through the builtin driver (memory and localfs alike; 4 MiB fine, 8 MiB already crashes, Debug build) kills the process with SIGSEGV. The gdb backtrace nests `put_object → ByteCountingReader::read → CountingBodyReader → Sha256VerifyingReader → StallGuardReader → SocketBodyReader::read → …` over and over: builtin's `SocketBodyReader::read` completes synchronously and the backend's next `co_await body.read()` is issued **inside** the previous read's final-suspend resume chain, one stack frame per 64 KiB until it overflows. beast/httplib are unaffected (their reads complete through an executor). Same root as the ASan `StreamPrefetch` entry in §2: `Task` resumes the continuation with `resume()` on synchronous completion instead of symmetric transfer | `core/task.h`: symmetric transfer for the "inner already completed" case (`await_suspend` returning the continuation handle), or a trampoline in builtin's `SocketBodyReader`; afterwards grow the e2e hot-added-backend stream object back to 32 MiB and restore the strict "close after the stream ends" assertion | high | medium |
