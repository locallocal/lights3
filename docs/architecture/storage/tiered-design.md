...
docs/architecture/storage/storage-backend.md §2 有意把路由停在 bucket 粒度，并预留"object 级分层用叠加实现、
...
（TierIndex / TierScanner 是设计层名字：实现里对应 `tier_access_table.h` 的
...
- **在途读者不受影响**：步骤 c 是 rename 覆盖，正在 `pread` 的读者持旧 inode
  fd，继续读完完整旧数据（docs/architecture/object-read-write-flow.md §3.2 的 fd 快照语义天然保护）；
...
range），响应按 docs/architecture/object-read-write-flow.md §3.1 正常走 206。可配置
...
backends:
  ...
  - name: tiered
    type: tiered
    local: localdata                  # localfs/xlocalfs 或 duostore（§2）
    cloud: aws                        # 任意后端
    cold_after: 30d                   # 判冷阈值
    scan_interval: 1h
...
| 故障 | 行为 |
| --- | --- |
| 云端不可达（GET remote） | 透传云端错误映射（对齐 cloudproxy：远端 5xx → InternalError(500)/SlowDown(503)，不引入 502，见 cloudproxy-design.md §5.1）；本地 `local`/`cached` 对象完全不受影响 |
...
`lights3 tier quarantine list|forget|purge <backend> …`（[cli.md §2.4](../../usage/cli.md)）：
...
（[monitoring.md](../../usage/monitoring.md) "tiered 水位"、[storage/tiered.md](tiered.md) 指标表）。
