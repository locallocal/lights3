...
> English translation of [../../storage/duostore-meta-redis-design.md](../../../architecture/storage/duostore-meta-redis-design.md). The Chinese original is authoritative; section numbering matches.
...
- ~~The **data-plane** cross-process issues of multi-gateway shared meta are not
  solved in this document~~ — implemented: the read / write leases and the GC
  lease live in this store (§2.2 `gc_lease` / `readlease:<owner>`, main doc §11
  `read_lease`, [duostore-core.md](../../../architecture/storage/duostore-core.md) §8.5).
...
check types: eq (redis.sha1hex(HGET) == expected; the client sends the sha1 of the old raw bytes) / absent (HGET is nil)
...
op types:   hset / hdel / hincr / zadd / zrem / del / set / pub (PUBLISH, §3.6)
...
| delete_bucket | `buckets[b]` exists; `o:<b>` hlen0; `up:<b>` hlen0 | HDEL `buckets` + DEL `oz:<b>` + DEL `uz:<b>` (the emptiness check covers in-progress multipart, aligned with AWS, same argument as the RocksDB version) |
...
- **Publish inside the commit script**: the guarded-commit op table gains a `pub`
  ...
  `swap_extents` append `PUBLISH <prefix>inv "<origin>\0<bucket>\0<key>"` as the batch's last
...
2. Cheap mitigation: **on process start and on the first reservation of each
   segment counter in the process lifetime, burn one extra segment** (not
   re-burned on reconnect)—skipping ids that may have been
...
`std::unique_ptr<redisReply, RedisReplyDeleter>`. Errors are layered, uniformly
via `throw_internal(what, ...)` / `check_reply_error` (modeled on the RocksDB version's
...
(if the uri carries a password) → `SELECT` (if not db 0); scripts are
server-level and need no per-connection reload (NOSCRIPT self-heals per §3.5).
...
| Key | Default | Notes |
| --- | --- | --- |
| meta | `rocksdb` | `rocksdb` / `redis` / `sqlite` / `tikv`; choosing redis without `LIGHTS3_DUOSTORE_REDIS_META` compiled in → configuration error |
...
1. **Interfacing the meta store suite**: the meta cases are extracted into
   `run_meta_store_suite(factory)` in `tests/unit/meta_store_suite.h` (modeled
   on the `backend_suite.h` pattern); the redis cases live in
   `tests/unit/test_duostore_redis.cc`; RocksMetaStore always runs,
...
2. **Obtaining a real redis** (docker availability not assumed): at test
   ...
   private instance on a unix socket under a temp directory
   (`redis-server --port 0 --unixsocket <tmp>/redis.sock --save '' --appendonly no --dir <tmp>`, killed at
