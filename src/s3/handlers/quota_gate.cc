...
bool S3Service::is_root(std::string_view access_key) const { return cred_store_ && cred_store_->is_root(access_key); }

Task<std::optional<uint64_t>> S3Service::existing_size(storage::IStorageBackend& backend, const std::string& bucket,
...
void S3Service::note_usage(const std::string& bucket, int64_t d_objects, int64_t d_bytes, int64_t d_mpu_bytes) {
    if (usage_) usage_->apply(bucket, d_objects, d_bytes, d_mpu_bytes);
}

Task<uint64_t> S3Service::upload_parts_bytes(storage::IStorageBackend& backend, const std::string& bucket,
...
void S3Service::check_quota(const std::string& bucket, int64_t add_bytes, int64_t add_objects,
    ...
    auto reject = [&](bool tenant_scope, const std::string& scope_name, const char* axis, int64_t have,
                      uint64_t limit) {
        usage_->quota_rejected(tenant_scope);
        std::string detail = std::string(axis) + " " + std::to_string(have) + " + request > " + std::to_string(limit) +
                             " (" + scope_name + ")";
        AuditEvent e;
        ...
        throw S3Error(S3ErrorCode::QuotaExceeded,
                      "The request would exceed the " + scope_name + " quota: " + detail + ".", bucket);
    };
    ...
    if (tenants_) {
        ...
        if (t->quota.max_bytes && tu.total_bytes() + add_bytes > static_cast<int64_t>(t->quota.max_bytes))
            reject(true, scope, "bytes", tu.total_bytes(), t->quota.max_bytes);
        if (t->quota.max_objects && tu.objects + add_objects > static_cast<int64_t>(t->quota.max_objects))
...
Task<void> S3Service::require_tenant_bucket(const std::string& bucket, std::string_view tenant, bool creating) {
    ...
    throw S3Error(S3ErrorCode::AccessDenied,
                  "Access denied: bucket is not owned by tenant '" + std::string(tenant) + "'.", bucket);
}
