...
| S3 Tables（Iceberg REST Catalog） | 设计稿 [s3-tables-design.md](s3-tables-design.md)、实施文档 [s3-tables/](s3-tables/README.md)；**①②③④ 已实现（2026-09-12）**：目录、表桶、提交协议、守卫、policy / 租户门、`s3tables` 签名名、凭证下发、lifecycle 排除、Avro 深校验、`catalog/diagnostics` / `recovery`、rename 恢复、`fsck` 对账、LoadTable ETag、维护 plan / run / purge 与周期 runner、`lights3-ctl tables`，单测与 e2e 段。下一步 ⑤ 多网关与文档 |
