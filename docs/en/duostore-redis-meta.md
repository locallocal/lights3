...
| `uploads` | `up:<b>` | HASH: field=`<key>\0<id>`, value=`encode_upload` | `list_uploads` = `HSCAN` in batches (COUNT 512, R4—huge uploads tables are no longer materialized whole in one command; cursor weak consistency is acceptable for ListMultipartUploads, and the client-side map doubles as dedup) + sort by field byte order, yielding the (key, upload_id) order (same order as the RocksDB prefix scan) |
