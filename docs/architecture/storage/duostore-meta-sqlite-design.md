...
- 分库分表不做；~~进程内在线备份 API（`sqlite3_backup`）维持不做~~——已实现
  （backlog ⑧：`backup_physical` 全量 + `sqlite_wal_archive` WAL 段增量，§6，
  [duostore-meta-sqlite.md](duostore-meta-sqlite.md) §10）。
...
-- S3 Tables 目录的 KV 门面（kv_*），每次打开 IF NOT EXISTS 补建
...
| `tc`（S3 Tables KV 门面） | `tc(key BLOB PRIMARY KEY, val BLOB)` 表 | 每次打开 `IF NOT EXISTS` 补建；`kv_*` 为逐语句点查 / 前缀扫，见 [duostore-meta-sqlite.md](duostore-meta-sqlite.md) §4.1 |
...
小锁（锁序恒 `alloc_mu_` → `mu_`，预留期间持有 `mu_` 把进程内业务写者挡在
外面）、`kIdSegment = 4096` 照搬。预留 =
**专用 alloc 连接**上的单语句事务：
...
**写锁竞争**：进程内业务事务已被 `mu_` 挡开，预留 UPDATE 只会
与绕过 flock 的外部写者争 SQLite 单写者锁，先由 busy_timeout(5s) 吸收；
SQLITE_BUSY 即单语句明确未执行，**有界重试（≤4 轮，每轮自带 5s 等待）而非
立刻 500**，超限才抛 InternalError。
...
- **冷备路径**（本节即 §6.3 的引用点）：干净 `close()` 后（§5.3
  ...
  **在线备份（S4 曾评估为不做，backlog ⑧ 已实现）**：`backup_physical` 以
  `sqlite3_backup_init/step/finish` 在进程内取全量一致快照
  （`supports_physical_backup()` = true），配置 `sqlite_wal_archive` 后开启备份链：
  自动 checkpoint 关闭（`wal_autocheckpoint=0`）、WAL 按段归档为增量备份并在
  close 时显式 TRUNCATE checkpoint，`restore_physical` 按链回放；未配链时
  仍是上述默认 checkpoint 策略。外部工具 `VACUUM INTO` / `.backup` 仍可用
  （WAL 允许外部只读连接并发）。详见 [duostore-meta-sqlite.md](duostore-meta-sqlite.md) §10。
...
近期 release tag（当前 `version-3.54.0`）。`build.sh` 的 `LIGHT_MODULES`
...
| sqlite_wal_archive | 空 | 备份链目录：`duostore backup --incremental` 的 WAL 段归档到这里，链一旦开始即关自动 checkpoint、关闭时补归档最后一段；空 = 只能全量（[storage/duostore-meta-sqlite.md §10](duostore-meta-sqlite.md#10-备份链与-pitr)） |
...
wal_archive, metrics}` 对齐 `RocksMetaOptions` 形态；读池大小复用默认 ≈ 线程池规模，不单独开配置键
（需要时再加，避免键膨胀）。
...
| S4 | 打磨：崩溃模拟（kill 后 WAL 回放对账）与一致视图注入专项、`sqlite3_backup` 在线备份评估（当时不做；backlog ⑧ 已实现，§6）、`PRAGMA optimize`/checkpoint 调优（`journal_size_limit`，§6）、指标（BUSY/corruption 计数）、文档状态头更新 | 全 ctest 矩阵绿 | 已完成（2026-07-30） |
