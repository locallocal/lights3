// L2: ?quota subresource (docs/architecture/multi-tenancy.md §3). No AWS
...
Task<http::HttpResponse> S3Service::get_bucket_quota(std::string bucket, const RequestAuth& auth) {
    (void)auth;
    auto snap = quota_store_ ? quota_store_->snapshot() : QuotaStore::Snapshot{};
    const auto* q = QuotaStore::find(snap, bucket);
    if (!q) throw S3Error(S3ErrorCode::NoSuchQuotaConfiguration, "The bucket has no quota configuration", bucket);
...
Task<http::HttpResponse> S3Service::put_bucket_quota(http::HttpRequest& req, std::string bucket,
    ...
    if (!quota_store_)
        throw S3Error(S3ErrorCode::InvalidRequest, "Bucket quotas are not available on this deployment.");
    if (!usage_ || !usage_->enabled())
        throw S3Error(S3ErrorCode::InvalidRequest, "Bucket quotas require usage accounting (usage.enabled).");
    ...
    std::string detail = "max_bytes=" + std::to_string(q.max_bytes) + " max_objects=" + std::to_string(q.max_objects);
...
Task<http::HttpResponse> S3Service::delete_bucket_quota(std::string bucket, const RequestAuth& auth) {
    require_root_for_quota(is_root(auth.access_key));
    if (!quota_store_)
        throw S3Error(S3ErrorCode::InvalidRequest, "Bucket quotas are not available on this deployment.");
