...
std::string TlsIdentityTraits::serialize(const Entry& b) {
    ...
    j["created_unix"] = std::chrono::duration_cast<std::chrono::seconds>(b.created.time_since_epoch()).count();
    if (!b.created_by.empty()) j["created_by"] = b.created_by;
    return j.dump();
}

std::optional<TlsBinding> TlsIdentityTraits::deserialize(const std::string& subject, const std::string& body) {
    try {
        json j = json::parse(body);
        if (!j.is_object() || !j.contains("access_key") || !j["access_key"].is_string()) return std::nullopt;
        ...
        b.created = std::chrono::system_clock::time_point(std::chrono::seconds(j.value("created_unix", int64_t{0})));
...
std::string TlsIdentityTraits::decode_key(const std::string& suffix) { return util::percent_decode(suffix); }
