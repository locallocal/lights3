...
> English translation of [../credential-management.md](../../architecture/credential-management.md). The Chinese original is authoritative; section numbering matches.

> Status: both phase 1 and phase 2 are implemented (phasing in §9, phase-2 design in §10). Builds on the `ICredentialProvider` extension point reserved in docs/architecture/s3-protocol.md §3.5.
...
- STS temporary credentials / credential rotation expiry — STS session
  credentials (`AssumeRole`) landed later (§9.1, [s3-protocol.md §3.5](s3-protocol.md));
  rotation expiry of permanent credentials is still not done;
...
`cred` command group: `cred list` / `cred get <ak>` / `cred create` /
`cred delete <ak>` map onto the GET / GET / POST / DELETE rows above (the `PUT`
in-place edit has no CLI wrapper yet), plus the certificate bindings
`cred bind-cert` / `cred unbind-cert` / `cred list-certs` ([tls.md §2.1](../usage/tls.md));
all sign with SigV4 themselves. The root AK/SK come from `--ak=`/`--sk=` or the
env vars `LIGHTS3_ADMIN_AK`/`LIGHTS3_ADMIN_SK` (prefer env for the SK: argv is
visible to local `ps`); options must follow the leaf subcommand; long
options accept both `--name=value` and `--name value` (ccmd v0.0.2); `cred get` supports
`--show-secret`, `cred create` supports `--comment` and `--policy` (inline
JSON or `@file`). Full reference: [cli.md §3.2](../usage/cli.md), or
...
- Credentials are written to a bucket named `.sys` on the `default_backend`,
  ...
  percent-encoded into one path segment, [tls.md §2.1](../usage/tls.md));
...
docs/architecture/s3-protocol.md §3.5:
...
### 9.1 Narrowed sessions for S3 Tables (2026-09-12)

The third parameter of `CredentialStore::mint_session(parent, ttl, narrow)`
narrows the session policy to `narrow_policy(parent policy, narrow)`
(`s3/auth/policy.h`): buckets / prefixes are kept item by item only where the
parent allows them, `readonly` is OR-ed, actions are intersected, and an empty
intersection is `AccessDenied` — a session never exceeds its parent credential.
Credential vending of the Iceberg REST catalog
([s3-tables-design.md §8.4](s3-tables-design.md)) is currently the only caller:
the session is confined to one table's data prefix and its metadata directory.
The `Policy` parameter of the AssumeRole form is still not supported. The
persisted format is unchanged (`.sys/sts/<ak>` already carries `policy`).
...
- Existing AKs are re-fetched only when their storage ETag differs from the
  one last seen: `PUT /-/admin/credentials/{ak}` (§2) edits policy/comment in
  place and bumps `rev`, and peers re-read the object on the next sync round;
  the SK itself stays immutable;
...
- How it is carried: the JSON body of `POST /-/admin/credentials`
  ...
  body wins), or the `policy` field of a credentials_file entry; editable
  after creation via `PUT /-/admin/credentials/{ak}` (§2, `"policy": null` clears);
