...
> 全绿。承接 docs/architecture/storage/storage-backend.md §4 的概述与 docs/architecture/storage/tiered-design.md §10 P5 的预留
> （tiered 的 cloud 侧接入真实云端）。本文档确定实现路线为**自签 SigV4 +
> vendored httplib 直连**（docs/architecture/storage/storage-backend.md §4.1 的路线 B）。
...
| 实现 `IStorageBackend` 全接口 | 含 bucket CRUD、对象数据面、list、multipart 全套（docs/architecture/storage/storage-backend.md §1，以 `src/storage/backend.h` 为准） |
| 全链路流式 | GET/PUT 不在内存缓冲整对象，背压传导到 TCP 层（延续 docs/architecture/storage/storage-backend.md §4 原则） |
| 凭证隔离 | 客户端用网关本地 AK/SK 认证；网关用自己的云凭证访问远端；两者绝不混淆透传 |
| 支撑 tiered P5 | 作为 docs/architecture/storage/tiered-design.md TieredBackend 的 cloud 侧后端（首要消费方），验收清单见 §9 |
...
docs/architecture/storage/storage-backend.md §4.1 原首选路线 A（aws-sdk-cpp 封装）。本设计**反转为路线 B**，理由：
...
4. **线程模型可控**：自实现可与项目协程/ThreadPool 模型精确融合；SDK 的
   同步/异步 API 反而要做第二次适配（docs/architecture/storage/storage-backend.md §4 已指出同步 SDK 占线程的瓶颈）。

代价：自动凭证链需自行实现（已实现，见 §1/§7 的凭证链，`aws_credentials.h`）；S3 协议边角（如 §4.4 complete 的
...
docs/architecture/concurrency.md §3.1 预留的通用 per-backend 独立池（`io_threads` 配置键）
...
③ ① 拿到 meta 后 co_return ObjectStream{meta,
       make_unique<PumpBodyReader>(queue, len /* 析构即 cancel */), range}
```
...
- **工具缺口**：响应里 `LastModified` 是 ISO8601，`core/util/time.h` 现只有
  `iso8601()` 格式化——P1 已补 `parse_iso8601()`；
...
| 远端表现 | 本地行为 |
| --- | --- |
| 4xx + 可解析 XML `<Error><Code>` | 按 wire code 反查 `S3ErrorCode` 原样透传（NoSuchKey / NoSuchBucket / NoSuchUpload / InvalidRange / InvalidPart / BucketNotEmpty / BucketAlreadyOwnedByYou / EntityTooLarge…）；反向查找为 `s3/errors.h` 的 `code_from_wire()` |
...
| 500 / 502 / 504、体不可解析的 5xx | `InternalError`（本地 500）。**不引入 502**：S3 错误词表本无 BadGateway，标准 S3 客户端把 500/503 视为可重试，保持协议忠实（docs/architecture/storage/storage-backend.md §4 原"502/503"表述随本文档修订） |
...
- 单段 PUT / upload_part：远端 ETag = 内容 MD5。§3.2 推流时已用
  ...
  的完整性补偿，也直接满足 docs/architecture/storage/tiered-design.md §5.2 "云端返回 etag 与本地内容校验"的依赖；
...
delete_tagging，数据面 get / put / upload_part。
...
## 9. 与 TieredBackend 的对接验收（docs/architecture/storage/tiered-design.md P5）

作为 tiered 的 cloud 侧后端，验收清单：

1. `put_object` 携带 `user_meta`（`x-amz-meta-lights3-*` 冗余头，docs/architecture/storage/tiered-design.md §4.2）
   上传后，`head_object` / `get_object` 能原样取回；
2. put / upload_part / complete 返回的 etag 非空，单段 = 内容 MD5
   （docs/architecture/storage/tiered-design.md §5.2 步骤 ③ 的校验依赖）；
3. Range GET 三种形态正确（docs/architecture/storage/tiered-design.md §6.3 透传依赖）;
4. head 返回 size / etag / last_modified 齐全（docs/architecture/storage/tiered-design.md §6.1 条件请求依赖）；
5. `list_objects` 可用于 docs/architecture/storage/tiered-design.md §9 对账遍历；
6. 远端不可达时抛 `InternalError` / `SlowDown` 而非挂死（docs/architecture/storage/tiered-design.md §9 故障矩阵
   依赖可预期的异常）。
...
| P4 | 重试/退避、超时细化、指标、日志；无长度 body 路径决断（落盘 spool 后定长上传，不做 TRAILER 组帧，§3.2）；`control_in_pump` 压测定默认值 | 故障注入专项测试过 | ✅ 全部落地（2026-07-31）：指标见 §8.2；`control_in_pump` 压测定默认 false（§2.3）；顺带实现 `force_path_style: false` vhost（§7） |
| P5 | e2e 双实例脚本；tiered 对接（§9 清单）；docs/architecture/storage/tiered-design.md P5 状态更新 | e2e 过；tiered + cloudproxy 冒烟过 | ✅（`e2e_cloudproxy` + `e2e_tiered_cloudproxy`） |
