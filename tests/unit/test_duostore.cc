// Dedicated DuoStore unit tests (docs/architecture/storage/duostore-design.md §14): codec roundtrip and run boundaries,
...
// interfaced out as meta_store_suite (docs/architecture/storage/duostore-meta-redis-design.md §9); RocksMetaStore
// always runs here, redis/sqlite/tikv run conditionally in their own test files. Compaction / crash-injection specials
// added with P4.
...
TEST(duostore_extent_run_roundtrip) {
    ...
    // kRados merges isomorphically to kChunk (docs/architecture/storage/duostore-data-rados-design.md §3.1):
    // consecutive ids compress into a single run; adjacent extents of different kinds do not merge
...
// Single-instance execution gating for multiple gateways (C4, docs/architecture/storage/duostore-data-rados-design.md
// §8.3): gc_enabled=false only stops the scheduling of the background worker/orphan scan; the manual hooks (test/ops
// channel) are not gated
