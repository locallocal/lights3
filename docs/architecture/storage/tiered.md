...
gauge；启动三个定时器（§7）。此外 `range_cache: true` 而
`local_->supports_range_cache()` 为假时打一条启动 WARN（Range GET 透传，§11）。
...
- **持久记录**：`tier_local.h:AccessRec{atime, hits, enrolled}`，由 `ITierLocal`
  ...
  文件系统无 xattr 时退回适配器私有的常驻表（`LocalFsTierLocal::table_`，
  `unordered_map<ikey, AccessRec>` + `table_dirty_`，`flush_access` 自行写
  `<state>/atime.tsv` 快照；旧格式两字段快照可直接加载，且在 xattr 模式下作为
  升级期的只读兜底）；duostore 适配器恒为常驻表（`tier_access_table.h:AccessTable`，
  meta 引擎没有原地改元数据的原语）。
...
`consider(cx, bucket, key, from_slot=-1)`；结束时 `sweep_range_cache()` 清理块缓存残留并把
其占用计入账本，校准 `local_bytes_est_`，记录 `last_full_scan_`。

**增量轮（`scan_incremental`）**：`wheel_slots()` 升序取槽文件，只处理
`slot <= now/3600` 的到期槽，逐行 `consider(cx, bucket, key, from_slot=slot)`
（轮内去重），处理完的槽文件删除——仍热的键已在 `consider` 里重新登记到后面的槽。

**`consider`（两种模式共用的单键裁决）**：签名
`consider(ScanCtx&, bucket, key, int64_t from_slot)`，`from_slot >= 0` 即来自
时间轮（`from_wheel`）。`local_->read`——不存在则（增量轮）计 `stale`；`remote` 且 `local_bytes > 0` → **崩溃恢复**候选（`demote_object`
补完 stub）；钉住（`cold_after_for < 0`）或本地 0 字节 → 跳过；
`now - access_of().atime >= cold_after` → **判冷**候选，size 计入 `cold_freed`；
否则（仍热）先把 `enrolled == from_slot` 的记录重置为 -1（正在消费的槽文件随后
...
HEAD 完全本地完成（stub 的 sidecar 信息完备），只额外 `touch`（§3 写后缓冲）。

`inspect_object`（`lights3-ctl object inspect`）同样只读本地：`local_->read` 得
分层视图（`tier` / `logical_size` / `local_bytes` / `local_mtime`，非 local 时加
`remote_etag` / `remote_at`）放在 `ObjectLayout.attrs` 前部，再把 local 引擎自己的
`inspect_object` 结果以 `local_engine` + `local.*` 前缀追加，extents 原样透传。
...
- `delete_object`：本地删除立即返回（**响应不等云端**），`forget_access`，云副本
  入 GC 异步删，块缓存丢弃；
- multipart 其余接口与 `set_object_tagging` 纯委托 local（进行中的分片不参与
  分层；stub 的元数据本就在本地侧，改 tagging 不碰云端）。
...
`touch`），无冗余头视为外来对象（远端 bucket 可能与他方共用），告警跳过
绝不动。
...
副本回来了则保留 stub、删条目、返回 false）。离线 CLI 见
[cli.md §2.4](../../usage/cli.md)；同一批轮次与账本在**运行中的网关**里经
admin jobs API（`src/app/admin_jobs.cc:run_tier_job` / `quarantine_ledger`）
暴露，`lights3-ctl tier scan|gc|reconcile [--no-wait|--status]`、
`lights3-ctl tier quarantine list`（cli.md §3.12）。
