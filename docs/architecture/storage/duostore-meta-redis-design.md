...
- ~~多网关共享 meta 时**数据面**的跨进程问题不在本文档解决~~——已实现：
  读写租约与 GC 租约落在本 store（§2.2 `gc_lease` / `readlease:<owner>`，
  主文档 §11 `read_lease`，[duostore-core.md](duostore-core.md) §8.5）。
...
check 类型：eq（redis.sha1hex(HGET) == expected，客户端传旧字节的 sha1）/ absent（HGET 为空）
           / exists（HEXISTS）/ hlen0（HLEN == 0）/ zcard0（ZCARD == 0）
           / sha1（HGETALL 按 field 数值序拼接后 redis.sha1hex == expected，§3.3）
op 类型：  hset / hdel / hincr / zadd / zrem / del / set / pub（PUBLISH，§3.6）
...
| delete_bucket | `buckets[b]` exists；`o:<b>` hlen0；`up:<b>` hlen0 | HDEL `buckets` + DEL `oz:<b>` + DEL `uz:<b>`（空检查覆盖进行中 multipart，对齐 AWS，与 RocksDB 版同一论证） |
...
- **脚本管理**：Lua 源码以 C++ raw string 常量内嵌（不部署 .lua 文件）；
  构造期 `SCRIPT LOAD` 一次记住 SHA（脚本是 server 级，不按连接加载），
  调用一律 `EVALSHA`；收到 NOSCRIPT 错误则重新 LOAD 后重发（server 重启 /
  SCRIPT FLUSH 自愈）；
...
- **发布点在提交脚本内**：守卫式提交的 op 表多一种 `pub`（`PUBLISH KEYS[kx] a`），
  ...
  `PUBLISH <prefix>inv "<origin>\0<bucket>\0<key>"` 作为最后一个 op 装进批——与写同一次原子执行，
...
2. 廉价缓解：**进程生命周期内每个号段计数器的首次预留额外空烧一个号段**
   （重连不再重烧）——
...
`redisReply` 统一以 `std::unique_ptr<redisReply, RedisReplyDeleter>`
包裹。错误分层，统一经 `throw_internal(what, ...)` / `check_reply_error`（仿 RocksDB 版
...
`AUTH`（若 uri 带密码）→ `SELECT`（若非 0 号库）；脚本为 server 级无需按
连接重载（NOSCRIPT 时按 §3.5 自愈）。重试
边界严格遵守 §3.5。
...
OFF 时配置 `meta: redis` 在 `from_params`
...
| 键 | 默认 | 说明 |
| --- | --- | --- |
| meta | `rocksdb` | `rocksdb` / `redis` / `sqlite` / `tikv`；未编译 `LIGHTS3_DUOSTORE_REDIS_META` 时选 redis → 配置错误 |
...
1. **meta store 套件接口化**：meta 用例抽成 `tests/unit/meta_store_suite.h`
   的 `run_meta_store_suite(factory)`（仿 `backend_suite.h` 模式），redis
   用例在 `tests/unit/test_duostore_redis.cc`；
...
2. **真实 redis 的获取**（不假设 docker 可用）：测试启动时探测
   `redis-server` 可执行文件——找到则以临时目录下的 unix socket 拉起私有实例
   （`redis-server --port 0 --unixsocket <tmp>/redis.sock --save '' --appendonly no --dir <tmp>`，
