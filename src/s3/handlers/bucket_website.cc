...
// §2.2). Root (static credential) only, same two-tier model as the admin plane:
    ...
    if (!w.routing_rules.empty()) {
        ...
        for (auto& r : w.routing_rules) {
            ...
            if (!r.key_prefix_equals.empty() || r.http_error_code_equals) {
                x.open("Condition");
                if (!r.key_prefix_equals.empty()) x.element("KeyPrefixEquals", r.key_prefix_equals);
                if (r.http_error_code_equals)
                    x.element("HttpErrorCodeReturnedEquals", static_cast<uint64_t>(r.http_error_code_equals));
                x.close();
            }
            x.open("Redirect");
            if (!r.protocol.empty()) x.element("Protocol", r.protocol);
            if (!r.host_name.empty()) x.element("HostName", r.host_name);
            if (r.replace_key_prefix_with) x.element("ReplaceKeyPrefixWith", *r.replace_key_prefix_with);
            if (r.replace_key_with) x.element("ReplaceKeyWith", *r.replace_key_with);
            if (r.http_redirect_code != 301) x.element("HttpRedirectCode", static_cast<uint64_t>(r.http_redirect_code));
            x.close();
            x.close();
        }
        x.close();
...
WebsiteBucket parse_website_xml(const std::string& body, std::string bucket) {
    ...
    if (auto* ra = root.find("RedirectAllRequestsTo")) {
        if (root.find("IndexDocument") || root.find("ErrorDocument") || root.find("RoutingRules"))
        ...
        if (w.redirect_all_host.empty())
            throw S3Error(S3ErrorCode::InvalidArgument, "RedirectAllRequestsTo.HostName is required.");
        ...
        if (!w.redirect_all_protocol.empty() && w.redirect_all_protocol != "http" && w.redirect_all_protocol != "https")
            throw S3Error(S3ErrorCode::InvalidArgument, "RedirectAllRequestsTo.Protocol must be http or https.");
        return w;
    }
    auto* idx = root.find("IndexDocument");
    if (!idx) throw S3Error(S3ErrorCode::InvalidArgument, "IndexDocument is required.");
    w.index_suffix = idx->get("Suffix");
    if (w.index_suffix.empty() || w.index_suffix.find('/') != std::string::npos)
        throw S3Error(S3ErrorCode::InvalidArgument, "IndexDocument.Suffix must be non-empty and must not contain '/'.");
    ...
    if (auto* rules = root.find("RoutingRules")) {
        for (auto& rr : rules->children) {
            ...
            if (auto* cond = rr.find("Condition")) {
                ...
                if (!code.empty()) {
                    try {
                        ...
                        throw S3Error(S3ErrorCode::InvalidArgument, "Invalid HttpErrorCodeReturnedEquals value.");
                    }
            ...
            if (!red) throw S3Error(S3ErrorCode::InvalidArgument, "Each RoutingRule must contain a Redirect.");
            r.protocol = red->get("Protocol");
            if (!r.protocol.empty() && r.protocol != "http" && r.protocol != "https")
                throw S3Error(S3ErrorCode::InvalidArgument, "Redirect.Protocol must be http or https.");
            ...
            if (!code.empty()) {
                try {
                    ...
                    throw S3Error(S3ErrorCode::InvalidArgument, "Invalid HttpRedirectCode value.");
                }
                if (r.http_redirect_code < 300 || r.http_redirect_code > 399)
                    throw S3Error(S3ErrorCode::InvalidArgument, "HttpRedirectCode must be a 3xx status code.");
            }
            // A Redirect with no effect at all (no host/protocol/key change) loops back
            // to the same resource — reject the no-op
            if (r.protocol.empty() && r.host_name.empty() && !r.replace_key_prefix_with && !r.replace_key_with)
                throw S3Error(S3ErrorCode::InvalidArgument,
        ...
        if (w.routing_rules.empty())
            throw S3Error(S3ErrorCode::InvalidArgument, "RoutingRules must contain at least one RoutingRule.");
    }
    return w;
}
...
static void require_root(const std::shared_ptr<CredentialStore>& store, std::string_view access_key) {
...
Task<http::HttpResponse> S3Service::get_bucket_website(std::string bucket, const RequestAuth& auth) {
...
Task<http::HttpResponse> S3Service::put_bucket_website(http::HttpRequest& req, std::string bucket,
    ...
    LOG_INFO("website: configuration for bucket {} set by {}", bucket, std::string(auth.access_key));
    co_return http::HttpResponse{};
}

Task<http::HttpResponse> S3Service::delete_bucket_website(std::string bucket, const RequestAuth& auth) {
    require_root(cred_store_, auth.access_key);
    if (!website_store_)
        throw S3Error(S3ErrorCode::InvalidRequest,
                      "Dynamic website configuration is not available on this deployment.");
    ...
    LOG_INFO("website: configuration for bucket {} deleted by {}", bucket, std::string(auth.access_key));
    http::HttpResponse resp;
    resp.status = 204;
    co_return resp;
}
