...
不实现该出口（见 [xlocalfs.md](xlocalfs.md) §6.1）。
...
- **head_object**（`localfs_backend.cc:head_object`）：入口校验 +
...
- **inspect_object**（`localfs_backend.cc:inspect_object`，roadmap §6.2）：
  ...
  tier(+remote_etag/remote_at)`，extents 恒一条 `file` 段；走 `load_object_meta`
  绕过缓存。
...
├── manifest            # TSV：bucket、key、content_type、checksum_algorithm、一等元数据、meta.*
├── part.00001          # 分片数据（part_file_name：part.%05d）
├── part.00001.md5      # TSV：md5=<该片 ETag>（+ checksum_algorithm/checksum_value，分片带已校验校验和时）；其存在 = 分片数据已持久
└── ...
```
...
- **complete**（`localfs_backend.cc:complete_multipart`）：
  ...
  3. 总 ETag = `combined_etag`（`md5(各分片 md5 二进制拼接)-N`，S3 规则），
     meta 来自 manifest，并记下 `meta.part_sizes`（各分片大小）；由 `.md5`
     里读回的各分片校验和经 `multipart.h:apply_composite_checksum` 算出
     COMPOSITE 校验和写入 meta 与 `PutResult`；取目标 key 的 commit_lock，
     走与 PUT 同一个 `commit_object_file` 原子提交；
...
- **超期清理**（`localfs_backend.cc:cleanup_stale_uploads`）：删除 manifest
  ...
  构造时扫一次，之后由 `schedule_periodic(mpu_timer_, …, &mpu_scan_task)`
  （§10）经 `TimerQueue` 每 `mpu_scan_interval_sec`（默认 6h）触发一次，完成后**再重挂**——扫描
...
| `fs_util.cc:prepare_object_dest` / `write_object_sidecar` | `commit_object_file` 拆出的两个同步半步（父目录创建 + 目录冲突检查 / 尾部 sidecar 写），供 xlocalfs `commit_prepared` 在中间插入 ring 上的 rename + 目录 fsync，以及 `defer_sidecar` 的后台补写 |
| `fs_util.cc:has_meta_xattr` | 数据文件是否带 `user.lights3.meta`（`inspect_object` 用） |
...
| `fs_util.cc:FdStreamReader` | pread 流式读 + `try_as_file` sendfile 出口（§4） |
...
入口 `lights3 fsck <backend>`（[../cli.md](../../usage/cli.md) §2.3）；xlocalfs 同
