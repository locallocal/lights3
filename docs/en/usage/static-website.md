...
conditional requests, Range, Cache-Control — see docs/architecture/s3-protocol.md); the
...
- **`PUT/GET/DELETE /bucket?website`** (AWS XML shape: `IndexDocument.Suffix`
  ...
  `lights3-ctl website get/set/delete <bucket>` ([cli.md §3.3](cli.md)).
...
## 6. Redirects and rate limiting (phase ④, roadmap §2.3)

- **RedirectAllRequestsTo** (HostName + optional Protocol; two entry points,
  XML and the YAML `redirect_all_host` / `redirect_all_protocol`): mutually
  exclusive with IndexDocument / ErrorDocument / RoutingRules (the AWS shape);
  every anonymous request to a matching bucket is 301-redirected to
  `<protocol>://<host>/<key>`, the protocol defaulting to the request scheme
  (`X-Forwarded-Proto`, http when direct).
- **RoutingRules** (XML API only, ≤50 rules, first match wins): Condition
  supports `KeyPrefixEquals` and `HttpErrorCodeReturnedEquals` (4xx/5xx);
  Redirect supports Protocol / HostName / ReplaceKeyPrefixWith / ReplaceKeyWith
  (the two Replace forms are mutually exclusive) / HttpRedirectCode (3xx,
  default 301). Rules with only a prefix condition are evaluated on the raw key
  **before** the object is fetched; rules with an error-code condition are
  evaluated when the error occurs, ahead of the 302 slash fix-up and the error
  document. With HostName and Protocol both absent the redirect is a path
  relative to this gateway (with the `/bucket` prefix in path-style).
- **Per-bucket anonymous rate limit**: `website[].max_rps` (YAML/JSON; the AWS
  XML has no such field, so it is not exposed through the `?website` API), see §2.
- **Metrics** (roadmap §5.3): `lights3_website_events_total{event}` —
  `anon_read` (requests entering the anonymous face), `index_rewrite` (key
  rewritten to the index document), `error_document` (error answered with the
  error document / built-in page), `redirect` (3xx from RedirectAllRequestsTo /
  RoutingRules / the 302 slash fix-up / `x-amz-website-redirect-location`),
  `throttled` (rejected by `max_rps`); together with
  `lights3_responses_by_status_total{status}` the 206/304 share can be read off.
