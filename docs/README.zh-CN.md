...
当前实现对应 [architecture.md](architecture/overview.md) 的架构。
...
是性能门禁与长稳跑——见 [testing.md](development/testing.md)。
...
运维 CLI `lights3-ctl`（凭证、桶网站配置、表桶、压测）与 `lights3` 完整命令树见 [cli.md](usage/cli.md)。
...
[s3-tables-design.md](architecture/s3-tables-design.md)；命令：[cli.md](usage/cli.md) §2.6 与 §3.13；
端点清单：[s3-protocol.md](architecture/s3-protocol.md) §1。

## 安装、打包、容器化

三条渠道，细节见 [deployment.md](usage/deployment.md)：
...
# 3. Docker / compose（docker/：Dockerfile、docker-compose.yml、镜像配置）
cd docker
...
- **HTTP 驱动**：四个驱动全部落地，运行期由 `http.driver` 切换、编译期由
  CMake 选项裁剪，并共享同一套驱动一致性测试（[http-adapter.md](architecture/http-adapter.md) §4 契约）：
...
- **凭证管理**（[credential-management.md](architecture/credential-management.md)）：
...
- **存储**：LocalFs（sidecar 元数据、staging+rename 原子写）、
  ...
  CloudProxy（自签 SigV4 直连远端 S3，[cloudproxy-design.md](architecture/storage/cloudproxy-design.md)）、
  Tiered（冷数据下沉云端的组合后端，[tiered-design.md](architecture/storage/tiered-design.md)）、
  DuoStore（元数据/数据分离引擎，meta 可选 RocksDB/Redis/SQLite/TiKV、
  data 可选本地 fs/RADOS，[duostore-design.md](architecture/storage/duostore-design.md)）；
  bucket 级 glob 路由
- **S3 API**：ListBuckets、Create/Head/DeleteBucket、Put/Get/Head/DeleteObject
  ...
  [static-website.md](usage/static-website.md)）；CORS（`?cors` + OPTIONS 预检 +
...
- **用量 / 配额 / 多租户 / 审计**（[multi-tenancy.md](architecture/multi-tenancy.md)）：
...
- **S3 Tables / Iceberg REST Catalog**（[s3-tables-design.md](architecture/s3-tables-design.md)）：
  表桶、`.sys` 上的目录状态与条件写提交 + 幂等重放、Avro 深校验、诊断 / 恢复、
  凭证下发、维护作业、views、多网关、可选的 DuoStore-meta 目录后备（见上文专节）

设计上明确不支持（返回 NotImplemented，见 [s3-protocol.md](architecture/s3-protocol.md) §1）：
versioning、ACL 细粒度（只认 private）、bucket policy、lifecycle
Transition/按 tag 过滤、SSE-C/KMS、Object Lock、presigned POST。

## 文档

文档按读者分组放在 [docs/](README.md)（中文原文）与 [docs/en/](en/README.md)
（英文镜像，章节编号一致；源码注释以 `docs/<group>/<name>.md §N` 形式引用章节），
两份索引都带每篇文档的一句话摘要。

| 分组（[中文](README.md) · [en](en/README.md)） | 文档 |
| --- | --- |
| **架构设计**——系统怎么构成、为什么这样做 | [overview](architecture/overview.md)、[http-adapter](architecture/http-adapter.md)、[concurrency](architecture/concurrency.md)、[coroutine-internals](architecture/coroutine-internals.md)、[object-read-write-flow](architecture/object-read-write-flow.md)、[s3-protocol](architecture/s3-protocol.md)、[credential-management](architecture/credential-management.md)、[multi-tenancy](architecture/multi-tenancy.md)、[s3-tables-design](architecture/s3-tables-design.md)、[storage/](architecture/storage/README.md)（storage-backend、tiered、cloudproxy、duostore 及其 redis / sqlite / tikv meta 与 rados data 引擎的设计文档，另有 13 篇实现级文档） |
| **使用**——部署、配置、运维 | [deployment](usage/deployment.md)、[cli](usage/cli.md)、[config-reload](usage/config-reload.md)、[tls](usage/tls.md)、[monitoring](usage/monitoring.md)、[static-website](usage/static-website.md)；配置键全在 [config/lights3.yaml](../config/lights3.yaml) 的注释里 |
| **开发**——构建、测试、贡献 | [contributing](development/contributing.md)、[testing](development/testing.md)、[performance-baseline](development/performance-baseline.md)、[todo](development/todo.md) |
| **归档**——已收口的历史底账，只读 | `docs/archive/`（roadmap、gaps、issues、backlog；源码注释中 `roadmap §N` / `backlog §N` 的出处） |
