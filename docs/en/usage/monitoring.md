...
([http-adapter.md §2.1](../architecture/http-adapter.md)). Deployments with the `root`
...
| | `Lights3PoolBacklogged` | shared pool backlogged 5m (the per-backend-pool criterion of [concurrency.md §3.1](../architecture/concurrency.md)) | warning |
...
| DuoStore | GC queue depth and head age (convergence criterion), rounds / reclaims / duration, skip reasons, pack live vs total, quarantine, corruption and orphans, the meta / data engine signals (sqlite / redis / tikv / rocksdb / rados) |
| Tiered | read source, demotion / promotion / eviction, local tier capacity (filesystem used vs high watermark, booked local bytes vs quota), cloud GC, scans and quarantine, range cache, op errors |
