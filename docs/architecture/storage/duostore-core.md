...
### 3.4 通用 KV 门面（`kv_*`，S3 Tables 目录后备）

对象表之外的一块不透明有序 KV 空间（[../s3-tables-design.md](../s3-tables-design.md)
§12），`tables.catalog_backing = duostore` 时作 S3 Tables 目录的后备；
`DuoStoreBackend::meta()` 把引擎暴露给目录层。落点：rocksdb CF `tc`、sqlite
表 `tc`、redis hash `tc`/`tce` + 字典序 ZSET `tcz`、tikv key 标签 `'T'`。

| 方法 | 语义 |
| --- | --- |
| `kv_get(key) -> optional<KvItem{key,value,etag}>` | 缺返回 nullopt |
| `kv_put(key, value, cond) -> etag` | 条件写；`meta_store.h:check_kv_condition` 在引擎原子区内执行（if_match 缺 → NoSuchKey、不符 → PreconditionFailed；if_none_match 已存在 → PreconditionFailed） |
| `kv_delete(key) -> bool` | 幂等，false = 缺 |
| `kv_scan(prefix, after, limit)` | 前缀升序，严格大于 `after`，limit=0 取引擎默认页；带 value |
| `kv_put_batch(span<KvPut>)` | 全或无多写，各项条件同一原子区内校验（目录的提交记录与指针同落）；按输入序返回新 ETag |

value 的 ETag = `meta_store.h:kv_etag`（sha256 hex 前 16 字符），由引擎计算并
在自身原子区内比较。基类默认全部抛 NotImplemented（无门面的引擎退回对象后备）。
...
共享引擎的基线契约是 TTL 有界陈旧；redis 之上另有失效广播（下段），tikv 无
等价物。指标 `lights3_meta_cache_lookups_total{result=hit|miss}`（duostore 不用校验谓词，无 stale）/
...
（`IMetaStore::subscribe_invalidations`，默认 false），配置键
`meta_cache_feed`（默认 true；false = 不订阅、退回纯 TTL 契约，运维总闸，
也是陈旧性测试的口径），计数 `lights3_duostore_meta_cache_feed_invalidations_total`
（对端提交消息丢弃的记录数）/ `lights3_duostore_meta_cache_feed_resets_total`
（（重）连接触发的整表清空次数）。细节见
...
`lights3 fsck <backend>`（[../cli.md](../../usage/cli.md) §2.3）。
...
幂等 → `shutdown_background`（`bg_.begin_close` → 组锁外 cancel 三个定时器
`gc_timer_` / `orphan_timer_` / `lease_timer_`（§8.5 租约发布器）
...
| counter | `meta_cache_feed_invalidations_total` / `meta_cache_feed_resets_total` | 跨网关失效广播（§7.1）：对端提交消息丢弃的缓存记录数 / 订阅（重）连接触发的整表清空次数 |
| gauge | `uring_fallback` | 仅 `data: fs` 且 `fs_uring: true` 而内核不可用时置 1（同步 IO 回落，[duostore-data-fs.md §9](duostore-data-fs.md)） |
