...
> English translation of [../../storage/duostore-meta-tikv-design.md](../../../architecture/storage/duostore-meta-tikv-design.md). The Chinese original is authoritative; section numbering matches.
...
- keyspace / API V2 multi-tenancy is not enabled in the first phase
  ...
  §3.2);
- ~~Data-plane cross-process issues not solved in this document~~ — implemented:
  the read / write leases and the GC lease live in this store (§3.2 `L` table,
  main doc §11 `read_lease`, [duostore-core.md](../../../architecture/storage/duostore-core.md) §8.5).
...
- Fetch one extra row to decide `is_truncated`; when truncation lands exactly on a
  ...
  skipped, not traversed): the sidecar wraps a key-only reverse
  `KvScan` (`TikvClient::last_key`, §6.1), so fetching the group's tail key costs
  O(1) RPCs — isomorphic to the RocksDB version's SeekForPrev;
...
## 5. alloc_file_run: Counter RMW Segments

(The interface is `alloc_file_run(kind, n)` for contiguous runs; `alloc_file_id` is the inline n=1 wrapper.)
...
1. Register this gateway's service safepoint = now − `tikv_gc_retention`
   (default 10 minutes; TTL max(3×interval, 60s), so a gateway whose advancement stalls is
...
- New files: `tikv_meta_store.{h,cc}` + `tikv_client.{h,cc}` (§8.3); the `DuoStoreBackend` constructor
