...
void run_tier_scan(const Cmd& c) {
    ...
    LOG_INFO(
        ...
        st.full ? "full" : "incremental", st.walked, st.full ? "objects walked" : "wheel candidates", st.cold_picked,
        st.recovered, st.enrolled, st.evicted, st.evicted_bytes, st.need_remaining);
    app.shutdown();
}
...
void run_tier_gc(const Cmd& c) {
    ...
    LOG_INFO(
        "tier admin: gc round: resolved {} (cloud replicas removed {}), deferred {}, "
        "failed {}",
...
void run_tier_reconcile(const Cmd& c) {
    ...
    LOG_INFO(
        "tier admin: reconcile round: {} cloud objects walked; stubs rebuilt {}, "
        "orphans deleted {}, orphans skipped {}, refs missing {}; quarantine new {}, "
        "resolved {}",
        st.cloud_objects, st.stubs_rebuilt, st.orphans_deleted, st.orphans_skipped, st.refs_missing, st.quarantined_new,
...
void run_tier_quarantine_list(const Cmd& c) {
    ...
    if (entries.empty()) {
        ...
        std::printf("%-13s %-20s %-40s %-34s %-20s %-20s %s\n", "KIND", "BUCKET", "KEY", "ETAG", "FIRST_SEEN",
                    "LAST_SEEN", "COUNT");
        for (const auto& e : entries)
            std::printf("%-13s %-20s %-40s %-34s %-20s %-20s %llu\n", e.kind.c_str(), e.bucket.c_str(), e.key.c_str(),
                        e.etag.c_str(), util::iso8601(std::chrono::system_clock::from_time_t(e.first_seen)).c_str(),
...
void run_tier_quarantine_purge(const Cmd& c) {
    ...
    if (sync_wait(t->quarantine_purge(bucket, key))) {
        ...
        LOG_WARN(
            "tier admin: {}/{} not purged (not a refs_missing finding, or the cloud copy "
...
Cmd make_tier_quarantine() {
    ...
    cmd->add_subcommand(make_backend_leaf(
        ...
        run_tier_quarantine_list));
    cmd->add_subcommand(make_backend_leaf("forget", "lights3 tier quarantine forget tierdata archive photos/2024/a.jpg",
    ...
    cmd->add_subcommand(
        make_backend_leaf("purge", "lights3 tier quarantine purge tierdata archive photos/2024/a.jpg",
...
Cmd make_tier() {
    ...
    cmd->add_subcommand(make_backend_leaf("gc", "lights3 tier gc tierdata",
    ...
    cmd->add_subcommand(make_backend_leaf(
        "reconcile", "lights3 tier reconcile tierdata", "lights3 tier reconcile <backend> [--config=<path>]",
