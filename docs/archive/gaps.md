...
> 时的快照；接续的规划底账 [roadmap.md](roadmap.md) 也已于 2026-09-05 收口归档于此，当前待办见 [../backlog.md](../backlog.md)。
>
...
- **第三部分 · 文档与实现的偏差**：15 条实现与承诺不符（含"改实现"与"改文档"两类，**已全部处理**），另有 7 条中英文档漂移（§8，**已全部修复**）。
...
回归）；第二部分至此清零。**§8 中英文档漂移 7 条与第三部分"文档与实现的偏差"15 条对照表均已于 2026-08-12 处理完毕**（详见各节逐行结论）。**全表至此清零**。
...
> 本表 15 行**已于 2026-08-12 逐行复核并处理完毕**：表格下方为逐行结论。
> 复核发现 12 行的"实际"列已被 §1–§7 的修复兑现（本表写于那些修复之前），
> 对它们只需标注证据与补文档措辞；真正动实现的仅 1 行（permit 生命期），
> 按"改文档承认不做"处置的 1 行（断连取消源），纯文档 2 行（builtin 收录、
> trailer 说明）。
...
**✅已处理**（2026-08-12，逐行；行序同表）：

1. **localfs LIST**：§2.7 修复属实——prefix 剪枝（`localfs_backend.cc:607-616`
   直接定位起始目录）、`subtree_may_match` 整子树跳过、delimiter 命中组后
   `skip_prefix` 免展开、非递归分层排序保证目录序即字典序。文档承诺已兑现，
   无需改动。
2. **取消三源**：超时与 shutdown 已接线（dispatch 的 `with_timeout` + main.cc
   关停源，汇入同一请求级 CancelSource）；**客户端断连按"改文档承认不做"处置**
   ——感知点在 socket 触碰处（body 读抛异常 / 响应写失败即丢弃），不触碰
   socket 的长 handler 由请求级超时兜底；独立侦测在同步驱动要每连接一个侦测
   线程、异步驱动要并发监听半关闭且不得误吞 pipelining 字节，成本不抵收益。
   `concurrency.md` §5 已改为如实表述。
3. **等待直方图**：§7 已输出 `lights3_pool_wait_seconds`；`concurrency.md`
   §3.1 两处"暂未经 /-/metrics 输出"的旧话已删——它们把运维者从正确判据推回
   `queue_depth` 粗信号，是"实现已好、文档反向拖后腿"的类型。
4. **permit 生命期（唯一动实现的行）**：main.cc 把 Permit 系进
   `stream_body`（最外层 `PermitBodyReader` 包装），驱动读完/断连丢弃响应体
   时才归还——`max_inflight_requests` 从"只约束 handler 协程帧"变为覆盖响应
   传输全程；关停排空按 `available()` 判在途，从此也数得到流式传输中的请求。
   小响应仍随协程帧归还（写一段内存的耗时有界，不值得把 permit 穿进驱动）。
5. **不支持功能的请求头**：§3.4/§3.5 修复属实（`reject_unsupported_headers`
   前缀+精确表，先于路由匹配）；文档 §1 清单补上 tagging 与
   storage-class（实现拒绝面本就大于旧清单）并写明"query 子资源 + 请求头"
   双覆盖。
6. **CreateBucket LocationConstraint**：§5.4 修复属实（`buckets.cc:56-72`
   解析并校验，region 不符 → InvalidLocationConstraint），文档两处表述与
   实现逐字一致，无需改动。
7. **条件请求原子性**：§2.6 下推属实（判定与提交同处后端原子提交点：localfs
   提交段 per-key 锁 / duostore 元数据事务 / cloudproxy 透传上游；L2 只余
   无锁快速失败预检）。`s3-protocol.md` §6 补上**原子性范围**说明：
   duostore/cloudproxy 跨实例成立；localfs/xlocalfs 互斥为进程内，共享文件
   系统多实例不在支持范围（architecture.md 非目标）。
8. **`/-/` 与 vhost**：§3.8 修复属实（`internal = !addr.vhost && …`，vhost 下
   `/-/metrics` 按普通对象键处理）；文档补明"该论断仅对 path-style 成立"与
   读端点只认 GET/HEAD。
9. **httplib 100-continue**：上游 API 限制（v0.20 无法延迟应答），按备选项
   "在文档显式声明为该驱动的已知降级"处置——`http-adapter.md` §3.2 新增
   已知降级清单（100-continue 立即应答 / 单行头编译期上限 / 未注册方法以
   405+S3 XML 拒绝），一致性测试按此放行。
10. **四驱动接受集合**：实现侧四条缺口均已在前几轮修复（httplib 整体头上限
    按配置补做、连接上限四驱动统一、builtin 双栈、seastar inet_address 收
    v4/v6 字面量）；`http-adapter.md` §4 新增契约 7 明文化"接受/拒绝集合
    一致"，并把 httplib 两处上游残留指向 §3.2 的已知降级。顺带补上契约 6
    （HEAD 框架头）——代码注释早已引用"契约 6"而文档清单只有 5 条。
11. **builtin 未收录**：`http-adapter.md` 补 §3.0（定位/模型/双栈/无 TLS），
    中英同步；`builtin_server.cc:2` 的 §3.2 与 `seastar_server.cc:1` 的 §3.4
    错位引用改为 §3.0/§3.3。
12. **tiered quota**：§6.2 修复属实（`note_local_delta` PUT/DELETE 就地增减 +
    `maybe_kick_quota_scan` 超水位提前踢 + 每轮 scan 实测校准），文档
    "遍历累计、增量维护"两项均属实，无需改动。
13. **查询返回 SK**：§5.10 修复属实（静态凭证恒掩码，`is_static()` 判定 +
    审计日志）；`credential-management.md` §2 取舍段补上静态凭证例外——
    "无法避免"的论证对配置来源的凭证不成立（指向 §10.5）。
14. **吊销与在途请求**：§3.7 修复属实（`VerifiedIdentity` 携带验签时刻
    policy 快照，不回 store 二次查表）；文档 §7 把"自然完成"点明为"按验签
    时刻的 policy 快照完成"。
15. **trailer 说明**：`http-adapter.md` §1 新增设计说明——trailer 不进中立
    模型，L1 有界读掉丢弃（`http.trailer_max_size`），S3 流式校验和走 body
    内 aws-chunked（`STREAMING-*-TRAILER` 的 trailer 在 aws-chunked 帧内由
    L2 解析），通用 HTTP trailer 无 S3 消费方。
...
3. ~~**3.1 / 3.2 / `AsyncSemaphore` 取消是同一条链**（2.9 已单独修完）：接线取消之前必须先修 3.2（回调不在触发线程跑续体）与信号量取消支持（§2.13 只做了析构断言，`close()` 留在这里），否则接上 `with_timeout` 反而引入"定时器线程跑请求"的新故障模式。~~ ✅ 2026-08-07/08 完成（§3 整节，§4 随后）。
   ~~**下一站是 §5.1 的分页缺失**~~ ✅ 2026-08-09 完成（§5 整节）。~~**当前下一站是 §6 存储引擎能力缺口**~~ ✅ 2026-08-11 完成（§6 整节）。~~**当前下一站是 §7 运维与工程能力缺口**~~ ✅ 2026-08-12 完成（§7 整节 + §6.3 的 cloudproxy 保留桶回归）。~~剩余：第三部分"文档与实现的偏差"与 §8 中英文档漂移~~ ✅ 均已于 2026-08-12 完成（§8 + 第三部分 15 条对照表逐行复核处理）。**gaps.md 全部条目至此清零**。
