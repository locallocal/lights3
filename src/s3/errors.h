...
// Code table size (same source as the enum): error counters use a fixed-size atomic array indexed by enum value,
// lock-free
...
struct S3Error : std::exception {
    ...
    // x-amz-bucket-region for redirect-class errors. Carried on the exception; L2 attaches them to the response during
    // unified rendering
