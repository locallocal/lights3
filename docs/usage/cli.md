...
先在 §1 说清，后文不再重复。启动装配流程见 [architecture.md §4](../architecture/overview.md#4-进程结构与启动流程)，
凭证管理面见 [credential-management.md](../architecture/credential-management.md)，
...
lights3 tables export|import <file> [--backing=object|duostore] [--config=<path>]  S3 Tables 目录在两种后备间迁移（§2.6）
...
| `--backing=<name>` | `tables export|import` | `tables.catalog_backing` | 读 / 写哪种目录后备：`object` 或 `duostore`；其它值退出 2 |
...
[storage/duostore-core.md §11](../architecture/storage/duostore-core.md#11-meta-dumpload)）。
...
- `duostore gc <backend>`：一轮完整 GC（mpu_ttl 清理 → gcq 消费 → 按龄封存 +
  压实 → 整空 pack 删除，[storage/duostore-core.md §8.1](../architecture/storage/duostore-core.md)）。
...
- `tier quarantine list <backend>`：打印隔离区账本（kind / bucket / key / etag /
  ...
  见 [tiered-design.md §9](../architecture/storage/tiered-design.md)。
...
S3 Tables 目录状态在两种后备之间迁移（[s3-tables-design.md §12](../architecture/s3-tables-design.md)，
...
| 选项 | 默认 | 说明 |
| --- | --- | --- |
| `-e, --endpoint=<url>` | `http://127.0.0.1:9000` | `scheme://host[:port]`；https 需要带 OpenSSL 的构建。服务端配置了 `http.admin_port` 时，admin 监听只服务 `/-/` 路径、数据面端口对其答 404，反之亦然：走 `/-/admin/*` 的 `cred` / `tenant` / `usage` / `reload` / `object` / `fsck --offline|--status` / `duostore` / `tier` / `tables plan|run` 必须指向 **admin 端口**；走 S3 面或 `/iceberg/v1` 的 `website`（`?website`）/ `quota`（`?quota`）/ `mpu` / `bench` / 在线 `fsck <bucket>` / 其余 `tables` 子命令走**数据面端口**（[http-adapter.md §2.1](../architecture/http-adapter.md)） |
...
[multi-tenancy.md §4.4](../architecture/multi-tenancy.md)）；其它凭证调用会得到 403。
...
[credential-management.md §11](../architecture/credential-management.md)。
...
操作 `?quota` 子资源（[multi-tenancy.md §3](../architecture/multi-tenancy.md)）。`set` 整体
...
操作 `/-/admin/tenants`（[multi-tenancy.md §6](../architecture/multi-tenancy.md)）。变更
...
读取 `/-/admin/usage`（[multi-tenancy.md §2](../architecture/multi-tenancy.md)）。root 看
...
[s3-tables-design.md](../architecture/s3-tables-design.md) 的运维入口（实现记录见
[s3-tables-design.md §16 ④](../architecture/s3-tables-design.md)）。目录调用打
