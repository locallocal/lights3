...
（[http-adapter.md §2.1](../architecture/http-adapter.md)）。开了 `root` 门禁的部署，要么用
会签名的旁路代理抓取，要么把监听放在私网并保持匿名。TLS 部署改
`scheme: https` 并配置 `tls_config`。

> **默认是匿名的，而这个端点不只有数字**：桶名、每桶请求量与字节数、后端名与拓扑都在
> 里面。推荐的落地形态是**把 `/-/` 面放到私网监听**：
>
> ```yaml
> http:
>   bind: 0.0.0.0          # 数据面对外
>   admin_bind: 127.0.0.1  # /-/metrics 与 /-/admin 只在本机（或私网网段）
>   admin_port: 9100
> ```
>
> 这样抓取照旧匿名、不用改 Prometheus，而对外端口对 `/-/` 一律 404。另一条路是
> `metrics_access: root`，但它要求抓取方会签名（Prometheus 原生不会），且
> **认证本身关闭时它无效**。两者都没做时，启动日志会 WARN 一行点名这件事 —— 判据是
> "服务 `/-/` 的那个监听地址不是回环"，所以 `bind: 127.0.0.1` 的本机实验不会被唠叨。
...
| | `Lights3PoolBacklogged` | 共享池积压 5m（[concurrency.md §3.1](../architecture/concurrency.md) 的分池判据） | warning |
...
| DuoStore | GC 队列深度与队头年龄（收敛判据）、轮次/回收/耗时、跳过原因、pack live vs total、隔离、损坏与孤儿、meta / data 引擎信号（sqlite / redis / tikv / rocksdb / rados） |
