...
| [s3-tables-design.md](s3-tables-design.md) | S3 Tables / Apache Iceberg REST Catalog（①–⑥ 全部已实现，2026-09-12）：RustFS 实现调研结论、建在 `.sys` 上的目录状态与 `PutCondition` CAS 提交协议、REST 端点与错误模型、Avro 深校验与诊断/恢复、表桶守卫与凭证下发、维护作业与 `lights3-ctl tables`、多网关矩阵、PyIceberg / DuckDB 冒烟、Iceberg views、`/_iceberg` 别名、duostore-meta 目录后备 |
| [s3-tables/](s3-tables/README.md) | S3 Tables 实施文档（①–⑥ 已实现，各步末尾有实现记录）：设计 §14 六个步骤各一份实现级文档——文件清单、数据结构与签名、逐步流程、现有代码接入点、配置、单测清单、陷阱 |
