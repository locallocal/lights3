...
| GET 流式读 | **替换**：`xlocalfs_backend.cc:UringStreamBodyReader` → `uring_stream.h:UringReadStream` | read-ahead 流水线（§5.1），Range 天然支持；不提供 localfs 的 sendfile 出口（§6.1） |
| HEAD / inspect_object / 元数据缓存 | 复用 localfs | `meta_from_stat` 回填与 `invalidate_on_exit` 守卫同基类（[localfs.md](localfs.md) §5/§5.1） |
...
依赖政策（见 [../architecture.md](../overview.md) §6）不引入 liburing：
...
延迟指标止于流句柄就绪。与 localfs 的一处差异：`UringStreamBodyReader`
不实现 `try_as_file`，故 localfs `FdStreamReader` 的 sendfile 零拷贝出口
（[localfs.md](localfs.md) §4）在 xlocalfs 上不生效，body 恒经 ring 读进
用户缓冲再写 socket。
...
（`md5(各分片二进制 md5 拼接)-N`）复用基类；同样记下 `meta.part_sizes` 并由
`.md5` 读回的分片校验和经 `apply_composite_checksum` 得出 COMPOSITE 校验和
（写入 meta 与 `PutResult`），与 localfs 的 §7 第 3 步一致。
...
（`shutdown_background`：取消 mpu 清理与孤儿 sidecar 扫描两个定时器，并等
在途扫描与 async sidecar 补写收尾，见 [localfs.md](localfs.md) §10）。
`uring.cc:UringRing::shutdown`（幂等，析构也调，
...
localfs 的全套选项（`registry.cc:fs_backend_opts`：root/staging/mpu_ttl/
mpu_scan_interval/require_xattr/sidecar/list_meta_concurrency/
list_cache_entries/list_cache_min_dir_entries/sidecar_scan_interval/
meta_cache_entries/meta_cache_ttl/meta_cache_validate）之上增加（映射
