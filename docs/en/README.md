...
| [s3-tables-design.md](s3-tables-design.md) | S3 Tables / Apache Iceberg REST Catalog (①–⑤ implemented, 2026-09-12): findings from the RustFS implementation, catalog state on `.sys` with a `PutCondition` CAS commit protocol, REST endpoints and error model, deep Avro validation with diagnostics / recovery, table-bucket guard and credential vending, maintenance jobs and `lights3-ctl tables`, the multi-gateway matrix, PyIceberg / DuckDB smoke |
| `s3-tables/` | S3 Tables implementation documents (①–⑤ implemented, implementation notes at the end of each step, Chinese only): one implementation-level document per step of design §14 — file list, data structures and signatures, step-by-step flows, integration points in existing code, configuration, unit-test list, pitfalls |
