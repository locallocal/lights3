...
分层职责见 [architecture.md](overview.md)，各层内部细节见
...
S3Service::dispatch()                       src/s3/service.cc
  ...
  │    '.' 开头 bucket 为内部保留名，统一拒绝（docs/architecture/credential-management.md §4.1）
  │ ⑥ per-credential policy 授权：按匹配路由的动作判定 policy 快照
  │    （GET/HEAD 为读，其余为写；CopyObject/UploadPartCopy 另对源桶做一次
  │    读授权，见 docs/architecture/credential-management.md §10.4）
  ▼
S3Service::route()                          显式分派表（docs/architecture/s3-protocol.md §2）
...
驱动线程上 `sync_wait_pumping`（该线程兼职泵队列），异步驱动挂到自己的事件循环
（docs/architecture/concurrency.md §4）。
...
2. 条件请求（docs/architecture/s3-protocol.md §6）：`If-None-Match: *` 先 `head_object` 探在，存在则 412
...
- 客户端断连由驱动层 reader 以异常上抛（契约见 docs/architecture/http-adapter.md §4），
...
- **Multipart**（详见 docs/architecture/storage/storage-backend.md §3.2、docs/architecture/s3-protocol.md §1）：`upload_part` 与 PUT 完全同构
...
构造 FdStreamReader(fd, offset=f, remaining=len)   ← fd 所有权移交 reader
```

`FdStreamReader.read()` 每次先 `co_await pool_->schedule()` 再 `pread` 一块
...
3. 流式响应循环：整个循环是一个由 `sync_wait_pumping` 驱动的协程
   （`builtin_server.cc:stream_body`），`StreamPrefetch` 双缓冲预取下一块的同时把
   当前块 `send`，直到 EOF；定长明文的文件 body 先尝试 `sendfile(2)` 快路径
