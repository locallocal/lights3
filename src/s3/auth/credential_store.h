// L2: dynamic credential management (docs/architecture/credential-management.md): AK/SK generation/lookup/revocation +
// IStorageBackend persistence. Three-source model: static credentials (config file) = root; file credentials
// (credentials_file hot reload) and dynamic credentials (generated by this class) are data-plane only and may carry a
// per-credential policy (§10).
#pragma once
...
public:
    ...
    static Task<std::shared_ptr<CredentialStore>> load(std::shared_ptr<storage::IStorageBackend> backend,
    ...
    void authorize(std::string_view ak, std::string_view bucket, std::string_view key, Action action) const;

    // ---- Admin plane (docs/architecture/credential-management.md §5.1) ----
    ...
    // thread, and unlocking std::mutex across threads is UB); uniqueness of concurrent generate is guaranteed by the AK
    // random space tenant/tenant_admin: tenancy attributes (docs/architecture/multi-tenancy.md §4); the tenant id is
    // validated by the caller against the tenant registry
    Task<CredentialInfo> generate(std::string comment, std::optional<CredentialPolicy> policy = std::nullopt,
    ...
    // narrow (docs/architecture/s3-tables-design.md §8.4): the session policy becomes
    ...
    // Do one .sys incremental sync immediately: new AKs are pulled into the table, vanished dynamic credentials are
    // removed
    Task<void> sync_now();
