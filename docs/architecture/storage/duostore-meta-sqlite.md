...
对象表的建表 DDL 在 `sqlite_meta_store.cc:kSchemaDdl`（`init_schema` 在一个事务里建表 +
种子 + 盖章）；KV 门面表 `tc` 的 DDL 单独放在 `kKvDdl`，见本节末：
...
- **第九张表 `tc`**（KV 门面，S3 Tables 目录后备，§4.1）：
  `kKvDdl` = `CREATE TABLE IF NOT EXISTS tc(key BLOB PRIMARY KEY, val BLOB NOT NULL)
  WITHOUT ROWID, STRICT`，**不在 `kSchemaDdl`/`init_schema` 内**，而是构造函数每次
  打开都在写连接上执行一次（`IF NOT EXISTS`）——门面之前建的库无需 schema 升版即
  自动补表。
...
（docs/architecture/s3-tables-design.md §12；契约见 rocksdb 文档 §6.1）落在 `tc` 表，
value 原样存 BLOB，ETag 由共享的 `meta_store.h:kv_etag` 计算。写侧与业务提交同模型：
`kv_put_batch` 持 `mu_`、单事务内逐项 `kKvGet` 读现值 → `meta_store.h:check_kv_condition`
（抛出即 Txn 回滚，整批不落）→ `kKvPut`（INSERT OR REPLACE），一次 `commit()`；
`kv_put` 是单项批；`kv_delete` 持 `mu_` 事务内先查后 `kKvDel`，缺失返回 false。
读侧走读池：`kv_get` = `kKvGet` 点查；`kv_scan` 按 `after` 是否为空选
`kKvScanGe`（`key>=prefix`）/ `kKvScanGt`（`key>after`），`LIMIT` = limit（0 取 1000），
读到越出 prefix 的 key 即断。
...
- **`snapshot()`**（在线 dump，roadmap §3.7）：
  ...
  `get_object` / `list_objects` / `pack_stats` 全部转发到以连接为参数的
  `sqlite_meta_store.h` `*_in(Conn&)` 读体（活方法每次调用新租连接，视图始终用同一条），
  写连接照常提交互不阻塞。成员序 `lease_` 先于 `txn_`，析构先回滚事务再还连接。
