...
- ~~数据面跨进程问题不在本文档解决~~——已实现：读写租约与 GC 租约落在本
  store（§3.2 `L` 表，主文档 §11 `read_lease`，[duostore-core.md](duostore-core.md) §8.5）。
...
- 多取一条判 `is_truncated`；截断恰落在 delimiter 组上时 next_token 须
  取组内最后一条 key（组已被跳过、未逐条经过）：侧车封装了 key-only 反向
  `KvScan`（`TikvClient::last_key`，§6.1），对该组取尾 key 为 O(1) 次 RPC——与
  RocksDB 版的 SeekForPrev 同构；
...
## 5. alloc_file_run：计数器 RMW 号段

（接口为 `alloc_file_run(kind, n)` 连续号段，`alloc_file_id` 是 n=1 的内联包装。）
...
1. 注册本网关 service safepoint = now − `tikv_gc_retention`（默认
   10 分钟；TTL max(3×interval, 60s)，推进停摆的网关过两轮自动摘除）；
...
- 新文件 `tikv_meta_store.{h,cc}` + `tikv_client.{h,cc}`（§8.3）；`DuoStoreBackend` 构造分支加
