...
struct IMetaStore {
    ...
    // 资源分配与 GC 记账（§9）
    virtual uint64_t alloc_file_run(Extent::Kind kind, uint32_t n) = 0;  // 持久单调，连续号段（≤64）；alloc_file_id 为 n=1 的内联包装
...
（草图为节选，以 `meta_store.h` 为准：略去了 `head_object`、`put_object` 的
`PutCondition` 参数、pack 账 `seal_pack` / `drop_pack_stat` / `ack_reclaims`、
压实 `swap_extents_batch` / `scan_refs`、多网关租约 `try_gc_lease` /
`publish_lease` / `min_lease`、快照 `snapshot()`、S3 Tables `kv_*` 门面与
备份/恢复方法，见 [duostore-core.md](duostore-core.md) §4。）
...
struct WriteHint {
    ...
    std::string owner;                       // 写进 pack record 的归属（"bucket\0key" / "mpu\0<id>\0<part_no>"，§5.2）
};
...
（草图为节选，以 `data_store.h` 为准：略去了 `write_batch`、`remove_pack`、
`seal_aged_packs`、`scan_chunks` / `scan_packs`（孤儿扫描枚举）、`pack_write_locked`、
`stat_pack`；`Extent::Kind` 另有 `kRados = 2`。）
...
| `uploads` | `<bucket>\0<key>\0<upload_id>` | `{v2, initiated_ms, content_type, user_meta, 一级元数据}` | 前缀扫 `<bucket>\0` 天然按 (key, upload_id) 排序 |
| `parts` | `<bucket>\0<key>\0<id>\0<be16 part_no>` | `{v2, size, md5, modified_ms, extent_runs, checksum_algorithm/checksum_value}` | big-endian part_no 保证升序，list_parts 前缀扫即得 |
| `refs` | `<be64 chunk_file_id>` | owner 简述（调试用） | chunk 存活引用表；孤儿判定 O(1) |
| `gcq` | `<be64 seq>` | `{extents, reason, enqueue_ms}` | 待回收队列 |
| `stats` | `p<be64 pack_id><b\|r\|s>` / `c0` `c1` `q` | merge 计数器 | pack 存活账（`b`/`r` = live_bytes/live_recs 增量 merge，`s` = sealed 标记）；`c0`/`c1` = chunk/pack file_id 号段，`q` = gcq seq 号段 |
| `tc` | S3 Tables 键 | 不透明 | S3 Tables 目录的 KV 门面（`kv_*`） |
...
**public 手动钩子 `Task<DuoGcStats> run_gc_once()`**（测试直调，仿 tiered
P1"手动触发下沉"先例）；孤儿扫描独立低频（`orphan_scan_interval` 默认
1d）+ 同款钩子 `run_orphan_scan_once()`；另有只读校验钩子 `run_scrub_once()`。GC 与业务共用 `pool_`，经 `core/semaphore.h` 限流，
压实拷贝分块进行不长占池线程。
...
- pack extent → 存活账已在业务事务扣减，直接销账；顺带检查
  `live_recs == 0` 且 sealed 的 pack → 再满足空置已满 `gc_grace`
  （`pack_empty_since_` 冷却）、未被 pin、且早于所有对端读租约后 → 整文件
  unlink + 删 packstat。
...
- 反向：refs 有而文件缺 → **告警计数，绝不静默删 meta**（数据丢失征兆，
  留人工介入）；
- pack 内未记账 record 即死区，压实自然回收；孤儿扫描另经
  `IDataStore::scan_packs` 枚举 `packs/`，无 packstat 行、过宽限、未 pin 且未被
  flock 持有的 pack 文件（建文件与首条 record 提交之间崩溃的残留）直接 unlink。
...
| GC 压实遇损坏 record | 跳过 + 告警，其余存活记录照常迁移；连续 3 轮（`kQuarantineStrikes`）有损坏且零迁移的 pack 进隔离区账本 `<root>/quarantine`（保留文件），人工经 `lights3 duostore quarantine list\|release\|purge` 处置 |
...
| pack_max_age | 1h | active pack 按龄封存（低写入量下死区才能进压实候选）；0 = 关 |
...
| 侧 | 实现 | 接入方式 |
| --- | --- | --- |
| meta | redis / TiKV（多网关共 meta，均已实现） | 实现 IMetaStore：同步客户端在池线程调用即可（§2.2）；事务不变量用各自原语（redis MULTI/Lua、TiKV 事务）表达——语义级接口不假设有序 KV，这正是 §2.1 选 A 的原因。Redis 版详细设计见 [duostore-meta-redis-design.md](duostore-meta-redis-design.md)，TiKV 版详细调研见 [duostore-meta-tikv-design.md](duostore-meta-tikv-design.md) |
| meta | SQLite（单文件部署，已实现） | 同上，SQL 事务。详细设计见 [duostore-meta-sqlite-design.md](duostore-meta-sqlite-design.md) |
| data | Ceph / RADOS（已实现） | 实现 IDataStore：新增 `Extent::Kind::kRados`（file_id 映射 rados 对象名），meta 层零改动；librados 异步 API 与协程接口天然契合。详细调研见 [duostore-data-rados-design.md](duostore-data-rados-design.md) |
| data | io_uring 版 FsDataStore（已实现，`fs_uring: true`） | 复用 xlocalfs 的 `UringEngine`/`uring_stream` 流水线，数据面换 IO 引擎、布局不变（[storage/duostore-data-fs.md](duostore-data-fs.md) §9） |

组合矩阵：RocksDB+本地盘（首期）、TiKV+Ceph（全分布式网关）、
RocksDB+Ceph（本地索引 + 远端数据）均为合法组合。注意：跨网关共享 meta
时 §7 的进程内 pin 表不再充分——已以读写租约 + GC 租约补齐（§11
`read_lease`，[duostore-core.md](duostore-core.md) §8.5）。
...
`if(LIGHTS3_DUOSTORE)` 内 `target_sources`（duostore 六个 .cc + `tier_local_duo.cc`）+
...
1. **一致性套件**：`tests/unit/test_storage.cc` 增 `#ifdef LIGHTS3_DUOSTORE`
   ...
   `chunk_size` 调小至 4KiB（强制多 chunk manifest）；另有 `fs_uring: true`
   的两个 `duostore_backend_suite_uring*` 变体（共五个套件）；
